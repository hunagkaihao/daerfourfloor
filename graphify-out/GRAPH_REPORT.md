# Graph Report - daerfourfloor  (2026-08-26)

## Corpus Check
- 1435 files · ~859,635 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10772 nodes · 24124 edges · 606 communities (458 shown, 148 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 987 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4a327585`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_ErpWorkstationMaterialRequest|ErpWorkstationMaterialRequest]]
- [[_COMMUNITY_IRepository|IRepository]]
- [[_COMMUNITY_CellDto|CellDto]]
- [[_COMMUNITY_ErpWorkshopMaterialTransfer|ErpWorkshopMaterialTransfer]]
- [[_COMMUNITY_Cell|Cell]]
- [[_COMMUNITY_ErpWorkstationMaterialReceiptDto|ErpWorkstationMaterialReceiptDto]]
- [[_COMMUNITY_ErpMaterial|ErpMaterial]]
- [[_COMMUNITY_IPickListService|IPickListService]]
- [[_COMMUNITY_StockService|StockService]]
- [[_COMMUNITY_AgvTaskManager|AgvTaskManager]]
- [[_COMMUNITY_.FindByIdAsync|.FindByIdAsync]]
- [[_COMMUNITY_SelectCall.vue|SelectCall.vue]]
- [[_COMMUNITY_CreateOutboundTaskFromBarcode.vue|CreateOutboundTaskFromBarcode.vue]]
- [[_COMMUNITY_Erp_StockEventHandler|Erp_StockEventHandler]]
- [[_COMMUNITY_BoxService|BoxService]]
- [[_COMMUNITY_ResponseDto|ResponseDto]]
- [[_COMMUNITY_ValueObject|ValueObject]]
- [[_COMMUNITY_IdInput|IdInput]]
- [[_COMMUNITY_SkipService|SkipService]]
- [[_COMMUNITY_StockService.cs|StockService.cs]]
- [[_COMMUNITY_MaterialSampling.vue|MaterialSampling.vue]]
- [[_COMMUNITY_ISkipService|ISkipService]]
- [[_COMMUNITY_PickListService|PickListService]]
- [[_COMMUNITY_TuTa.Wms.Application.Contracts.Shared|TuTa.Wms.Application.Contracts.Shared]]
- [[_COMMUNITY_TuTa.Wms.Erp.Entities|TuTa.Wms.Erp.Entities]]
- [[_COMMUNITY_BoxBind.vue|BoxBind.vue]]
- [[_COMMUNITY_Autos.vue|Autos.vue]]
- [[_COMMUNITY_TuTa.Wms.Erp|TuTa.Wms.Erp]]
- [[_COMMUNITY_PickList|PickList]]
- [[_COMMUNITY_AgvTaskPagedQueryDto|AgvTaskPagedQueryDto]]
- [[_COMMUNITY_Department|Department]]
- [[_COMMUNITY_CreateOutStockTaskSummary.vue|CreateOutStockTaskSummary.vue]]
- [[_COMMUNITY_ErpDeliveryStation|ErpDeliveryStation]]
- [[_COMMUNITY_ErpDbContext|ErpDbContext]]
- [[_COMMUNITY_RolePermissionAppService|RolePermissionAppService]]
- [[_COMMUNITY_WmsDbContext.cs|WmsDbContext.cs]]
- [[_COMMUNITY_TuTa.Wms.AgvTasks|TuTa.Wms.AgvTasks]]
- [[_COMMUNITY_IRoleAppService|IRoleAppService]]
- [[_COMMUNITY_IApplicationService|IApplicationService]]
- [[_COMMUNITY_ErpOutboundOrderDto|ErpOutboundOrderDto]]
- [[_COMMUNITY_ErpAsnAppService|ErpAsnAppService]]
- [[_COMMUNITY_StockConsolidationPlanner|StockConsolidationPlanner]]
- [[_COMMUNITY_ErpPickOrderSyncJob|ErpPickOrderSyncJob]]
- [[_COMMUNITY_ErpAsn|ErpAsn]]
- [[_COMMUNITY_MaterialService|MaterialService]]
- [[_COMMUNITY_EnumCheckType|EnumCheckType]]
- [[_COMMUNITY_PagedStockQueryDto|PagedStockQueryDto]]
- [[_COMMUNITY_WarehouseAreaDto|WarehouseAreaDto]]
- [[_COMMUNITY_ErpPickMan|ErpPickMan]]
- [[_COMMUNITY_StockConsolidationWorker|StockConsolidationWorker]]
- [[_COMMUNITY_WmsController|WmsController]]
- [[_COMMUNITY_IRecheckListService|IRecheckListService]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_IWarehouseService|IWarehouseService]]
- [[_COMMUNITY_OpenIddictDataSeedContributor|OpenIddictDataSeedContributor]]
- [[_COMMUNITY_RecheckItem|RecheckItem]]
- [[_COMMUNITY_IStockRepository|IStockRepository]]
- [[_COMMUNITY_ErpInboundOrder|ErpInboundOrder]]
- [[_COMMUNITY_SessionTimeoutLogin.vue|SessionTimeoutLogin.vue]]
- [[_COMMUNITY_StockConsolidationService|StockConsolidationService]]
- [[_COMMUNITY_BoxEventHandler|BoxEventHandler]]
- [[_COMMUNITY_AgvTaskService|AgvTaskService]]
- [[_COMMUNITY_ErpOutboundOrder|ErpOutboundOrder]]
- [[_COMMUNITY_NewAccountController|NewAccountController]]
- [[_COMMUNITY_IMaterialService|IMaterialService]]
- [[_COMMUNITY_PagingBase|PagingBase]]
- [[_COMMUNITY_WmsDbContext|WmsDbContext]]
- [[_COMMUNITY_Header.vue|Header.vue]]
- [[_COMMUNITY_StockInHistory_StockEventHandler|StockInHistory_StockEventHandler]]
- [[_COMMUNITY_RecheckList|RecheckList]]
- [[_COMMUNITY_IErpAsnAppService|IErpAsnAppService]]
- [[_COMMUNITY_ErpRecheckNotifierSyncJob|ErpRecheckNotifierSyncJob]]
- [[_COMMUNITY_ErpPickOrder|ErpPickOrder]]
- [[_COMMUNITY_IWarehouseRepository|IWarehouseRepository]]
- [[_COMMUNITY_TuTa.Wms.Materials.Aggregates|TuTa.Wms.Materials.Aggregates]]
- [[_COMMUNITY_TuTa.Wms.AgvTasks.Dto|TuTa.Wms.AgvTasks.Dto]]
- [[_COMMUNITY_ErpOutboundOrderAppService|ErpOutboundOrderAppService]]
- [[_COMMUNITY_WarehouseService|WarehouseService]]
- [[_COMMUNITY_ErpRecheckNotifier|ErpRecheckNotifier]]
- [[_COMMUNITY_ErpStockAftChk|ErpStockAftChk]]
- [[_COMMUNITY_Warehouse|Warehouse]]
- [[_COMMUNITY_EfCoreBoxRepository|EfCoreBoxRepository]]
- [[_COMMUNITY_TuTa.Wms.Stocks|TuTa.Wms.Stocks]]
- [[_COMMUNITY_TuTa.Wms.Users.Dtos|TuTa.Wms.Users.Dtos]]
- [[_COMMUNITY_TuTa.Wms.StockInHistories.ValueObjects|TuTa.Wms.StockInHistories.ValueObjects]]
- [[_COMMUNITY_EfCoreRepository|EfCoreRepository]]
- [[_COMMUNITY_UserAppService|UserAppService]]
- [[_COMMUNITY_ErpBarcode|ErpBarcode]]
- [[_COMMUNITY_Ware.vue|Ware.vue]]
- [[_COMMUNITY_ErpStateChgNotifier|ErpStateChgNotifier]]
- [[_COMMUNITY_TuTa.Wms.RecheckLists|TuTa.Wms.RecheckLists]]
- [[_COMMUNITY_TuTa.Wms.StockConsolidations|TuTa.Wms.StockConsolidations]]
- [[_COMMUNITY_WmsDbMigrationService|WmsDbMigrationService]]
- [[_COMMUNITY_.GetPagedMaterialsAsync|.GetPagedMaterialsAsync]]
- [[_COMMUNITY_ErpBarcodeSyncJob|ErpBarcodeSyncJob]]
- [[_COMMUNITY_ErpMove|ErpMove]]
- [[_COMMUNITY_AuditedEntityDto|AuditedEntityDto]]
- [[_COMMUNITY_ITransientDependency|ITransientDependency]]
- [[_COMMUNITY_Stock|Stock]]
- [[_COMMUNITY_ErpInboundOrderDto|ErpInboundOrderDto]]
- [[_COMMUNITY_ErpInboundOrderAppService|ErpInboundOrderAppService]]
- [[_COMMUNITY_AGVIncell.vue|AGVIncell.vue]]
- [[_COMMUNITY_Box|Box]]
- [[_COMMUNITY_Material|Material]]
- [[_COMMUNITY_EfCoreSkipRepository|EfCoreSkipRepository]]
- [[_COMMUNITY_ChkResultListEventHandler|ChkResultListEventHandler]]
- [[_COMMUNITY_IStockConsolidationService|IStockConsolidationService]]
- [[_COMMUNITY_RoleAppService|RoleAppService]]
- [[_COMMUNITY_.GetPagedStockInHistoriesAsync|.GetPagedStockInHistoriesAsync]]
- [[_COMMUNITY_ErpDeptType|ErpDeptType]]
- [[_COMMUNITY_AuditedAggregateRoot|AuditedAggregateRoot]]
- [[_COMMUNITY_PuArrVouchAddRequestDto|PuArrVouchAddRequestDto]]
- [[_COMMUNITY_StockEventHandler|StockEventHandler]]
- [[_COMMUNITY_home.vue|home.vue]]
- [[_COMMUNITY_ErpNoPlanPickType|ErpNoPlanPickType]]
- [[_COMMUNITY_Stock_RecheckListEventHandler|Stock_RecheckListEventHandler]]
- [[_COMMUNITY_ApplicationService|ApplicationService]]
- [[_COMMUNITY_BoxDiskWithAsn.vue|BoxDiskWithAsn.vue]]
- [[_COMMUNITY_StockOutHistory|StockOutHistory]]
- [[_COMMUNITY_ErpInboundOrderController|ErpInboundOrderController]]
- [[_COMMUNITY_StockConsolidationModels.cs|StockConsolidationModels.cs]]
- [[_COMMUNITY_TuTa.Wms|TuTa.Wms]]
- [[_COMMUNITY_ILocalEventHandler|ILocalEventHandler]]
- [[_COMMUNITY_BarcodeDto|BarcodeDto]]
- [[_COMMUNITY_EntityDto|EntityDto]]
- [[_COMMUNITY_TuTa.Wms.Application|TuTa.Wms.Application]]
- [[_COMMUNITY_TuTa.Wms.EntityFrameworkCore|TuTa.Wms.EntityFrameworkCore]]
- [[_COMMUNITY_TuTa.Wms.Domain.Shared|TuTa.Wms.Domain.Shared]]
- [[_COMMUNITY_GoodsBind.vue|GoodsBind.vue]]
- [[_COMMUNITY_IErpAsnRepository|IErpAsnRepository]]
- [[_COMMUNITY_IErpInboundOrderRepository|IErpInboundOrderRepository]]
- [[_COMMUNITY_IErpOutboundOrderRepository|IErpOutboundOrderRepository]]
- [[_COMMUNITY_.GetByBoxAsync|.GetByBoxAsync]]
- [[_COMMUNITY_ErpWarehouseAreaPrdt|ErpWarehouseAreaPrdt]]
- [[_COMMUNITY_.Configure|.Configure]]
- [[_COMMUNITY_TuTa.Wms.HttpApi|TuTa.Wms.HttpApi]]
- [[_COMMUNITY_PickListEventHandler|PickListEventHandler]]
- [[_COMMUNITY_EfCoreWarehouseRepository|EfCoreWarehouseRepository]]
- [[_COMMUNITY_StockInHistoryService|StockInHistoryService]]
- [[_COMMUNITY_StockOutHistoryService|StockOutHistoryService]]
- [[_COMMUNITY_TuTa.Wms.Domain|TuTa.Wms.Domain]]
- [[_COMMUNITY_TuTa.Wms.Application.Contracts|TuTa.Wms.Application.Contracts]]
- [[_COMMUNITY_RecheckListService|RecheckListService]]
- [[_COMMUNITY_WmsModuleExtensionConfigurator|WmsModuleExtensionConfigurator]]
- [[_COMMUNITY_WmsSettingDefinitionProvider|WmsSettingDefinitionProvider]]
- [[_COMMUNITY_HandOutCell.vue|HandOutCell.vue]]
- [[_COMMUNITY_CellEventHandler|CellEventHandler]]
- [[_COMMUNITY_BoxDisk.vue|BoxDisk.vue]]
- [[_COMMUNITY_IncellByPeople.vue|IncellByPeople.vue]]
- [[_COMMUNITY_WmsDomainModule.cs|WmsDomainModule.cs]]
- [[_COMMUNITY_IBarcodeListRepository|IBarcodeListRepository]]
- [[_COMMUNITY_ErpInboundOrderRequestDto|ErpInboundOrderRequestDto]]
- [[_COMMUNITY_LLBJDAddRequestDto|LLBJDAddRequestDto]]
- [[_COMMUNITY_CheckInfoOfChkRsltList|CheckInfoOfChkRsltList]]
- [[_COMMUNITY_IMaterialRepository|IMaterialRepository]]
- [[_COMMUNITY_AbpModule|AbpModule]]
- [[_COMMUNITY_TuTa.Wms.sln|TuTa.Wms.sln]]
- [[_COMMUNITY_WmsPermissionDefinitionProvider|WmsPermissionDefinitionProvider]]
- [[_COMMUNITY_ErpOutboundOrderRequestDto|ErpOutboundOrderRequestDto]]
- [[_COMMUNITY_StorageBox.vue|StorageBox.vue]]
- [[_COMMUNITY_ErpOutboundItem|ErpOutboundItem]]
- [[_COMMUNITY_ChkResultList|ChkResultList]]
- [[_COMMUNITY_TuTa.Wms.Events|TuTa.Wms.Events]]
- [[_COMMUNITY_WmsDomainService|WmsDomainService]]
- [[_COMMUNITY_HttpPost|HttpPost]]
- [[_COMMUNITY_.CreateCTUSkipStockInAsync|.CreateCTUSkipStockInAsync]]
- [[_COMMUNITY_.PreConfigureServices|.PreConfigureServices]]
- [[_COMMUNITY_MaterialStatusChange.vue|MaterialStatusChange.vue]]
- [[_COMMUNITY_.CreateContainer|.CreateContainer]]
- [[_COMMUNITY_.TrackIf|.TrackIf]]
- [[_COMMUNITY_CellInfo|CellInfo]]
- [[_COMMUNITY_WarehouseInfo|WarehouseInfo]]
- [[_COMMUNITY_WmsController.cs|WmsController.cs]]
- [[_COMMUNITY_TestModel.cs|TestModel.cs]]
- [[_COMMUNITY_CreateOutStockTask.vue|CreateOutStockTask.vue]]
- [[_COMMUNITY_AcceptanceCalltest.vue|AcceptanceCalltest.vue]]
- [[_COMMUNITY_.MoveStockAsync|.MoveStockAsync]]
- [[_COMMUNITY_BoxIncell.vue|BoxIncell.vue]]
- [[_COMMUNITY_CountInfoOfStock|CountInfoOfStock]]
- [[_COMMUNITY_WmsHttpApiModule|WmsHttpApiModule]]
- [[_COMMUNITY_StockController|StockController]]
- [[_COMMUNITY_linliaoNodeModal.vue|linliaoNodeModal.vue]]
- [[_COMMUNITY_MaterialBasic.vue|MaterialBasic.vue]]
- [[_COMMUNITY_CancellationToken|CancellationToken]]
- [[_COMMUNITY_DateTime|DateTime]]
- [[_COMMUNITY_Guid|Guid]]
- [[_COMMUNITY_IEnumerable|IEnumerable]]
- [[_COMMUNITY_ILogger|ILogger]]
- [[_COMMUNITY_List|List]]
- [[_COMMUNITY_AssemblyInfo.cs|AssemblyInfo.cs]]
- [[_COMMUNITY_ResponseDto|ResponseDto]]
- [[_COMMUNITY_ServiceConfigurationContext|ServiceConfigurationContext]]
- [[_COMMUNITY_AssemblyInfo.cs|AssemblyInfo.cs]]
- [[_COMMUNITY_AssemblyInfo.cs|AssemblyInfo.cs]]
- [[_COMMUNITY_StockConsolidationAgvQueryResult|StockConsolidationAgvQueryResult]]
- [[_COMMUNITY_StockConsolidationMovePlan|StockConsolidationMovePlan]]
- [[_COMMUNITY_StockConsolidationMoveResult|StockConsolidationMoveResult]]
- [[_COMMUNITY_StockConsolidationOptions|StockConsolidationOptions]]
- [[_COMMUNITY_StockConsolidationPalletSnapshot|StockConsolidationPalletSnapshot]]
- [[_COMMUNITY_StockConsolidationPlanner|StockConsolidationPlanner]]
- [[_COMMUNITY_StockConsolidationProgress|StockConsolidationProgress]]
- [[_COMMUNITY_StockConsolidationSnapshot|StockConsolidationSnapshot]]
- [[_COMMUNITY_StockConsolidationSnapshotResult|StockConsolidationSnapshotResult]]
- [[_COMMUNITY_StockConsolidationStatusDto|StockConsolidationStatusDto]]
- [[_COMMUNITY_Task|Task]]
- [[_COMMUNITY_IChkResultListRepository|IChkResultListRepository]]
- [[_COMMUNITY_SelectOrderDetail.vue|SelectOrderDetail.vue]]
- [[_COMMUNITY_MaterialPartialOut.vue|MaterialPartialOut.vue]]
- [[_COMMUNITY_Stock.vue|Stock.vue]]
- [[_COMMUNITY_CreateOrder.vue|CreateOrder.vue]]
- [[_COMMUNITY_Good.vue|Good.vue]]
- [[_COMMUNITY_AcceptanceCall.vue|AcceptanceCall.vue]]
- [[_COMMUNITY_People.vue|People.vue]]
- [[_COMMUNITY_Peoples.vue|Peoples.vue]]
- [[_COMMUNITY_.ModifyChkResultList|.ModifyChkResultList]]
- [[_COMMUNITY_CreateStockTask.vue|CreateStockTask.vue]]
- [[_COMMUNITY_MaterialOut.vue|MaterialOut.vue]]
- [[_COMMUNITY_Auto.vue|Auto.vue]]
- [[_COMMUNITY_LockPage.vue|LockPage.vue]]
- [[_COMMUNITY_LoginForm.vue|LoginForm.vue]]
- [[_COMMUNITY_Cell.vue|Cell.vue]]
- [[_COMMUNITY_MobileBrowser.vue|MobileBrowser.vue]]
- [[_COMMUNITY_LoginForm.vue|LoginForm.vue]]
- [[_COMMUNITY_CheckNoGet.vue|CheckNoGet.vue]]
- [[_COMMUNITY_StockConsolidation.vue|StockConsolidation.vue]]
- [[_COMMUNITY_ErpOutboundOrderController|ErpOutboundOrderController]]
- [[_COMMUNITY_LaneCellChips.vue|LaneCellChips.vue]]
- [[_COMMUNITY_AcceptanceCall2.vue|AcceptanceCall2.vue]]
- [[_COMMUNITY_task.vue|task.vue]]
- [[_COMMUNITY_Outstock.vue|Outstock.vue]]
- [[_COMMUNITY_TuTa.Wms.Warehouses.Dtos|TuTa.Wms.Warehouses.Dtos]]
- [[_COMMUNITY_OutStockOrder.vue|OutStockOrder.vue]]
- [[_COMMUNITY_AgvTaskManagement.vue|AgvTaskManagement.vue]]
- [[_COMMUNITY_TuTa.Wms.RecheckLists.Dtos|TuTa.Wms.RecheckLists.Dtos]]
- [[_COMMUNITY_TestScanAsn.vue|TestScanAsn.vue]]
- [[_COMMUNITY_ScanAsn.vue|ScanAsn.vue]]
- [[_COMMUNITY_StockQuery.vue|StockQuery.vue]]
- [[_COMMUNITY_BarcodeList.vue|BarcodeList.vue]]
- [[_COMMUNITY_StockAdjustment.vue|StockAdjustment.vue]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_ContainerUnbind.vue|ContainerUnbind.vue]]
- [[_COMMUNITY_OutStockSort.vue|OutStockSort.vue]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_MobileForm.vue|MobileForm.vue]]
- [[_COMMUNITY_RegisterForm.vue|RegisterForm.vue]]
- [[_COMMUNITY_.ImportMaterialBasicDataFromFileAsync|.ImportMaterialBasicDataFromFileAsync]]
- [[_COMMUNITY_EfCoreBarcodeListRepository|EfCoreBarcodeListRepository]]
- [[_COMMUNITY_ForgetPasswordForm.vue|ForgetPasswordForm.vue]]
- [[_COMMUNITY_AbpRole.vue|AbpRole.vue]]
- [[_COMMUNITY_LoginFormTitle.vue|LoginFormTitle.vue]]
- [[_COMMUNITY_ErpInboundItem|ErpInboundItem]]
- [[_COMMUNITY_MaterialInfoOfRechkList|MaterialInfoOfRechkList]]
- [[_COMMUNITY_IncellHis copy 2.vue|IncellHis copy 2.vue]]
- [[_COMMUNITY_OutcellHis.vue|OutcellHis.vue]]
- [[_COMMUNITY_AbpUser.vue|AbpUser.vue]]
- [[_COMMUNITY_AsnList.vue|AsnList.vue]]
- [[_COMMUNITY_Login.vue|Login.vue]]
- [[_COMMUNITY_Login.vue|Login.vue]]
- [[_COMMUNITY_IncellHis.vue|IncellHis.vue]]
- [[_COMMUNITY_OutboundOrder.vue|OutboundOrder.vue]]
- [[_COMMUNITY_WorkshopReceiptStatistics.vue|WorkshopReceiptStatistics.vue]]
- [[_COMMUNITY_createStockTask|createStockTask]]
- [[_COMMUNITY_Exception.vue|Exception.vue]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_MaterialStockStatistics.vue|MaterialStockStatistics.vue]]
- [[_COMMUNITY_YdStockQuery.vue|YdStockQuery.vue]]
- [[_COMMUNITY_App.vue|App.vue]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_OidcSignIn.vue|OidcSignIn.vue]]
- [[_COMMUNITY_OidcSignOut.vue|OidcSignOut.vue]]
- [[_COMMUNITY_EditCell.vue|EditCell.vue]]
- [[_COMMUNITY_ExcelOut.vue|ExcelOut.vue]]
- [[_COMMUNITY_FrameBlank.vue|FrameBlank.vue]]
- [[_COMMUNITY_CurrentCultureDto|CurrentCultureDto]]
- [[_COMMUNITY_ExtensionEnumDto|ExtensionEnumDto]]
- [[_COMMUNITY_IStringValueType|IStringValueType]]
- [[_COMMUNITY_ListResultDto_1OfOfUserDataAndAbstractionsAnd_0AndCulture_neutralAndPublicKeyToken_null|ListResultDto_1OfOfUserDataAndAbstractionsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PropertyApiDescriptionModel|PropertyApiDescriptionModel]]
- [[_COMMUNITY_TimeZone|TimeZone]]
- [[_COMMUNITY_ContainerUnbind.vue|ContainerUnbind.vue]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_TuTa.Wms.Domain.csproj|TuTa.Wms.Domain.csproj]]
- [[_COMMUNITY_TuTa.Wms.DbMigrator.csproj|TuTa.Wms.DbMigrator.csproj]]
- [[_COMMUNITY_TuTa.Wms.Application.Contracts.csproj|TuTa.Wms.Application.Contracts.csproj]]
- [[_COMMUNITY_Scrollbar.vue|Scrollbar.vue]]
- [[_COMMUNITY_AccountServiceProxy|AccountServiceProxy]]
- [[_COMMUNITY_ErpDeliveryStationServiceProxy|ErpDeliveryStationServiceProxy]]
- [[_COMMUNITY_PermissionsServiceProxy|PermissionsServiceProxy]]
- [[_COMMUNITY_index.d.ts|index.d.ts]]
- [[_COMMUNITY_WmsSettingDefinitionProvider|WmsSettingDefinitionProvider]]
- [[_COMMUNITY_HubMsgQHelper|HubMsgQHelper]]
- [[_COMMUNITY_CellEventHandler|CellEventHandler]]
- [[_COMMUNITY_2.7.0(httpsgithub.comanncwbvue-vben-admincomparev2.5.9...v2.7.0) (2021-08-03)|[2.7.0](https://github.com/anncwb/vue-vben-admin/compare/v2.5.9...v2.7.0) (2021-08-03)]]
- [[_COMMUNITY_functional.ts|functional.ts]]
- [[_COMMUNITY_WmsMenuContributor|WmsMenuContributor]]
- [[_COMMUNITY_Index.cshtml|Index.cshtml]]
- [[_COMMUNITY_ErpInboundOrderRequestDto|ErpInboundOrderRequestDto]]
- [[_COMMUNITY_LLBJDAddRequestDto|LLBJDAddRequestDto]]
- [[_COMMUNITY_IMaterialRepository|IMaterialRepository]]
- [[_COMMUNITY_dongfangdianzi|dongfangdianzi]]
- [[_COMMUNITY_dongfangdianzi|dongfangdianzi]]
- [[_COMMUNITY_ExpandTransition.ts|ExpandTransition.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_ProfileServiceProxy|ProfileServiceProxy]]
- [[_COMMUNITY_download.ts|download.ts]]
- [[_COMMUNITY_FakeCurrentPrincipalAccessor|FakeCurrentPrincipalAccessor]]
- [[_COMMUNITY_ErpOutboundOrderRequestDto|ErpOutboundOrderRequestDto]]
- [[_COMMUNITY_PuArrVouchAddRequestDto|PuArrVouchAddRequestDto]]
- [[_COMMUNITY_ChangePasswordInput|ChangePasswordInput]]
- [[_COMMUNITY_ErpInboundItem|ErpInboundItem]]
- [[_COMMUNITY_2.2.0(httpsgithub.comanncwbvue-vben-admincomparev2.1.1...v2.2.0) (2021-04-07)|[2.2.0](https://github.com/anncwb/vue-vben-admin/compare/v2.1.1...v2.2.0) (2021-04-07)]]
- [[_COMMUNITY_2.0.0-rc.15 (2020-12-31)|2.0.0-rc.15 (2020-12-31)]]
- [[_COMMUNITY_2.0.0-rc.15 (2020-12-31)|2.0.0-rc.15 (2020-12-31)]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_column.ts|column.ts]]
- [[_COMMUNITY_AbpTenantServiceProxy|AbpTenantServiceProxy]]
- [[_COMMUNITY_BoxBindCellEvent.cs|BoxBindCellEvent.cs]]
- [[_COMMUNITY_WmsTestDataSeedContributor|WmsTestDataSeedContributor]]
- [[_COMMUNITY_.WritePlcTag|.WritePlcTag]]
- [[_COMMUNITY_.PushCGRKDAddAsync|.PushCGRKDAddAsync]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_WmsAuthServerModule|WmsAuthServerModule]]
- [[_COMMUNITY_Program|Program]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_Program|Program]]
- [[_COMMUNITY_2.0.0-rc.10 (2020-11-13)|2.0.0-rc.10 (2020-11-13)]]
- [[_COMMUNITY_2.0.0-rc.14 (2020-12-15)|2.0.0-rc.14 (2020-12-15)]]
- [[_COMMUNITY_2.0.0-rc.6 (2020-10-28)|2.0.0-rc.6 (2020-10-28)]]
- [[_COMMUNITY_2.0.0-rc.7 (2020-10-31)|2.0.0-rc.7 (2020-10-31)]]
- [[_COMMUNITY_2.0.0-rc.9 (2020-11-9)|2.0.0-rc.9 (2020-11-9)]]
- [[_COMMUNITY_2.1.1 (2021-03-26)|2.1.1 (2021-03-26)]]
- [[_COMMUNITY_Breaking changes|Breaking changes]]
- [[_COMMUNITY_(Breaking changes) Breaking changes|(Breaking changes) Breaking changes]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY_2.0.0-rc.10 (2020-11-13)|2.0.0-rc.10 (2020-11-13)]]
- [[_COMMUNITY_2.0.0-rc.14 (2020-12-15)|2.0.0-rc.14 (2020-12-15)]]
- [[_COMMUNITY_2.0.0-rc.6 (2020-10-28)|2.0.0-rc.6 (2020-10-28)]]
- [[_COMMUNITY_2.0.0-rc.7 (2020-10-31)|2.0.0-rc.7 (2020-10-31)]]
- [[_COMMUNITY_2.0.0-rc.9 (2020-11-9)|2.0.0-rc.9 (2020-11-9)]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY_clickOutside.ts|clickOutside.ts]]
- [[_COMMUNITY_AbpLoginResult|AbpLoginResult]]
- [[_COMMUNITY_AgvCallBackRequest|AgvCallBackRequest]]
- [[_COMMUNITY_AgvTaskDto|AgvTaskDto]]
- [[_COMMUNITY_BarcodeDto|BarcodeDto]]
- [[_COMMUNITY_BoxAddDto|BoxAddDto]]
- [[_COMMUNITY_BoxUpdateDto|BoxUpdateDto]]
- [[_COMMUNITY_CellDto|CellDto]]
- [[_COMMUNITY_CellLaneStatusDto|CellLaneStatusDto]]
- [[_COMMUNITY_CellsBindAreaDto|CellsBindAreaDto]]
- [[_COMMUNITY_CellsDisBindFromAreaDto|CellsDisBindFromAreaDto]]
- [[_COMMUNITY_CGRKDAddRequestDto|CGRKDAddRequestDto]]
- [[_COMMUNITY_CGRKDParams|CGRKDParams]]
- [[_COMMUNITY_ChkResultListDto|ChkResultListDto]]
- [[_COMMUNITY_ErpDeliveryOrderCreateDto|ErpDeliveryOrderCreateDto]]
- [[_COMMUNITY_ErpDeliveryOrderDto|ErpDeliveryOrderDto]]
- [[_COMMUNITY_ErpDeliveryOrderItemCreateDto|ErpDeliveryOrderItemCreateDto]]
- [[_COMMUNITY_ErpDeliveryOrderItemDto|ErpDeliveryOrderItemDto]]
- [[_COMMUNITY_ErpDeliveryOrderListResponseDto|ErpDeliveryOrderListResponseDto]]
- [[_COMMUNITY_ErpDeliveryStationDto|ErpDeliveryStationDto]]
- [[_COMMUNITY_ErpDeliveryStationRequestDto|ErpDeliveryStationRequestDto]]
- [[_COMMUNITY_ErpInboundItemDto|ErpInboundItemDto]]
- [[_COMMUNITY_ErpInboundOrderRequestDto|ErpInboundOrderRequestDto]]
- [[_COMMUNITY_ErpInboundOrderResponseDto|ErpInboundOrderResponseDto]]
- [[_COMMUNITY_ErpLoginResponseDto|ErpLoginResponseDto]]
- [[_COMMUNITY_ErpMaterialResponseDto|ErpMaterialResponseDto]]
- [[_COMMUNITY_ErpOutboundItemDto|ErpOutboundItemDto]]
- [[_COMMUNITY_ErpOutboundItemRequestDto|ErpOutboundItemRequestDto]]
- [[_COMMUNITY_ErpOutboundOrderDto|ErpOutboundOrderDto]]
- [[_COMMUNITY_ErpOutboundOrderRequestDto|ErpOutboundOrderRequestDto]]
- [[_COMMUNITY_ErpOutboundOrderResponseDto|ErpOutboundOrderResponseDto]]
- [[_COMMUNITY_ErpOutboundRecordDto|ErpOutboundRecordDto]]
- [[_COMMUNITY_ErpPickManDto|ErpPickManDto]]
- [[_COMMUNITY_ErpWorkshopMaterialTransferDto|ErpWorkshopMaterialTransferDto]]
- [[_COMMUNITY_ErpWorkshopMaterialTransferRequestDto|ErpWorkshopMaterialTransferRequestDto]]
- [[_COMMUNITY_ErpWorkshopMaterialTransferResponseDto|ErpWorkshopMaterialTransferResponseDto]]
- [[_COMMUNITY_ErpWorkshopMaterialTransferStatusUpdateDto|ErpWorkshopMaterialTransferStatusUpdateDto]]
- [[_COMMUNITY_ErpWorkstationMaterialReceiptDto|ErpWorkstationMaterialReceiptDto]]
- [[_COMMUNITY_ErpWorkstationMaterialReceiptRequestDto|ErpWorkstationMaterialReceiptRequestDto]]
- [[_COMMUNITY_ErpWorkstationMaterialReceiptResponseDto|ErpWorkstationMaterialReceiptResponseDto]]
- [[_COMMUNITY_ErpWorkstationMaterialRequestDto|ErpWorkstationMaterialRequestDto]]
- [[_COMMUNITY_ErpWorkstationMaterialRequestRequestDto|ErpWorkstationMaterialRequestRequestDto]]
- [[_COMMUNITY_ErpWorkstationMaterialRequestResponseDto|ErpWorkstationMaterialRequestResponseDto]]
- [[_COMMUNITY_ErpWorkstationMaterialRequestStatusUpdateDto|ErpWorkstationMaterialRequestStatusUpdateDto]]
- [[_COMMUNITY_GetBySkipDto|GetBySkipDto]]
- [[_COMMUNITY_GetChkByBarcodeDto|GetChkByBarcodeDto]]
- [[_COMMUNITY_GetPermissionInput|GetPermissionInput]]
- [[_COMMUNITY_IdentityUserUpdateRolesDto|IdentityUserUpdateRolesDto]]
- [[_COMMUNITY_IdInput|IdInput]]
- [[_COMMUNITY_LoginInput|LoginInput]]
- [[_COMMUNITY_LoginOutput|LoginOutput]]
- [[_COMMUNITY_MaterialCreateDto|MaterialCreateDto]]
- [[_COMMUNITY_MaterialDto|MaterialDto]]
- [[_COMMUNITY_MaterialImportResultDto|MaterialImportResultDto]]
- [[_COMMUNITY_MaterialUpdateDto|MaterialUpdateDto]]
- [[_COMMUNITY_NoPlanPickListDelDto|NoPlanPickListDelDto]]
- [[_COMMUNITY_NoPlanPickListEditDto|NoPlanPickListEditDto]]
- [[_COMMUNITY_NoPlanPickTypeDto|NoPlanPickTypeDto]]
- [[_COMMUNITY_PagedCellsAreaDto|PagedCellsAreaDto]]
- [[_COMMUNITY_PagedCellsQueryDto|PagedCellsQueryDto]]
- [[_COMMUNITY_PagedCheckItemQueryDto|PagedCheckItemQueryDto]]
- [[_COMMUNITY_PagedMaterialsQueryDto|PagedMaterialsQueryDto]]
- [[_COMMUNITY_PagedPickItemQueryDto|PagedPickItemQueryDto]]
- [[_COMMUNITY_PagedResultDto_1OfOfErpWorkshopMaterialTransferDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfErpWorkshopMaterialTransferDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedResultDto_1OfOfErpWorkstationMaterialReceiptDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfErpWorkstationMaterialReceiptDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedResultDto_1OfOfErpWorkstationMaterialRequestDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfErpWorkstationMaterialRequestDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedResultDto_1OfOfSkipInDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfSkipInDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedResultDto_1OfOfStockInHistoryDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfStockInHistoryDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedResultDto_1OfOfTenantDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null|PagedResultDto_1OfOfTenantDtoAndContractsAnd_0AndCulture_neutralAndPublicKeyToken_null]]
- [[_COMMUNITY_PagedSkipDto|PagedSkipDto]]
- [[_COMMUNITY_PagedStockInHistoryQueryDto|PagedStockInHistoryQueryDto]]
- [[_COMMUNITY_PagedStockMoveQueryDto|PagedStockMoveQueryDto]]
- [[_COMMUNITY_PagedStockOutHistoryQueryDto|PagedStockOutHistoryQueryDto]]
- [[_COMMUNITY_PagedStockQueryDto|PagedStockQueryDto]]
- [[_COMMUNITY_PagedWarehouseQueryDto|PagedWarehouseQueryDto]]
- [[_COMMUNITY_PagingRoleListInput|PagingRoleListInput]]
- [[_COMMUNITY_PagingUserListInput|PagingUserListInput]]
- [[_COMMUNITY_PickItemDto|PickItemDto]]
- [[_COMMUNITY_PickItemQueryDto|PickItemQueryDto]]
- [[_COMMUNITY_PickStockAllocateDto|PickStockAllocateDto]]
- [[_COMMUNITY_PickStockDto|PickStockDto]]
- [[_COMMUNITY_PriorityStock|PriorityStock]]
- [[_COMMUNITY_PuArrVouchDetailRequestDto|PuArrVouchDetailRequestDto]]
- [[_COMMUNITY_RecheckItemDto|RecheckItemDto]]
- [[_COMMUNITY_RecheckItemQueryDto|RecheckItemQueryDto]]
- [[_COMMUNITY_RecheckStockDto|RecheckStockDto]]
- [[_COMMUNITY_RegisterDto|RegisterDto]]
- [[_COMMUNITY_ResetPasswordDto|ResetPasswordDto]]
- [[_COMMUNITY_ResultAgvTaskDto|ResultAgvTaskDto]]
- [[_COMMUNITY_ResultGetBySkipDto|ResultGetBySkipDto]]
- [[_COMMUNITY_SendPasswordResetCodeDto|SendPasswordResetCodeDto]]
- [[_COMMUNITY_SkipInDto|SkipInDto]]
- [[_COMMUNITY_SkipStockCtuInDto|SkipStockCtuInDto]]
- [[_COMMUNITY_StockCheckDto|StockCheckDto]]
- [[_COMMUNITY_StockCreateDto|StockCreateDto]]
- [[_COMMUNITY_StockDirectCreateDto|StockDirectCreateDto]]
- [[_COMMUNITY_StockDto|StockDto]]
- [[_COMMUNITY_StockMoveDto|StockMoveDto]]
- [[_COMMUNITY_StockOutHistoryDto|StockOutHistoryDto]]
- [[_COMMUNITY_StocksCtuInDto|StocksCtuInDto]]
- [[_COMMUNITY_TenantCreateDto|TenantCreateDto]]
- [[_COMMUNITY_TenantUpdateDto|TenantUpdateDto]]
- [[_COMMUNITY_UpdateProfileDto|UpdateProfileDto]]
- [[_COMMUNITY_UserLoginInfo|UserLoginInfo]]
- [[_COMMUNITY_VerifyPasswordResetTokenInput|VerifyPasswordResetTokenInput]]
- [[_COMMUNITY_WarehouseAreaDto|WarehouseAreaDto]]
- [[_COMMUNITY_WarehouseDto|WarehouseDto]]
- [[_COMMUNITY_WarehouseUpdateDto|WarehouseUpdateDto]]
- [[_COMMUNITY_AxiosCanceler|AxiosCanceler]]
- [[_COMMUNITY_WmsDomainService|WmsDomainService]]
- [[_COMMUNITY_AgvTaskDto|AgvTaskDto]]
- [[_COMMUNITY_CellInfo|CellInfo]]
- [[_COMMUNITY_WarehouseInfo|WarehouseInfo]]
- [[_COMMUNITY_2.0.0-rc.14(httpsgithub.comanncwbvue-vben-admincompare2.0.0-beta.3...v2.0.0-rc.14) (2020-12-15)|[2.0.0-rc.14](https://github.com/anncwb/vue-vben-admin/compare/2.0.0-beta.3...v2.0.0-rc.14) (2020-12-15)]]
- [[_COMMUNITY_2.0.0-rc.18(httpsgithub.comanncwbvue-vben-admincomparev2.0.0-rc.17...v2.0.0-rc.18) (2021-02-04)|[2.0.0-rc.18](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.17...v2.0.0-rc.18) (2021-02-04)]]
- [[_COMMUNITY_2.0.0-beta.5 (2020-10-10)|2.0.0-beta.5 (2020-10-10)]]
- [[_COMMUNITY_2.0.0-beta.7 (2020-10-12)|2.0.0-beta.7 (2020-10-12)]]
- [[_COMMUNITY_2.0.0-rc.11 (2020-11-18)|2.0.0-rc.11 (2020-11-18)]]
- [[_COMMUNITY_2.0.0-rc.1 (2020-10-14)|2.0.0-rc.1 (2020-10-14)]]
- [[_COMMUNITY_2.0.0-rc.2 (2020-10-17)|2.0.0-rc.2 (2020-10-17)]]
- [[_COMMUNITY_2.0.0-rc.3 (2020-10-19)|2.0.0-rc.3 (2020-10-19)]]
- [[_COMMUNITY_2.0.0-rc.4 (2020-10-21)|2.0.0-rc.4 (2020-10-21)]]
- [[_COMMUNITY_2.0.0-rc.5 (2020-10-26)|# 2.0.0-rc.5 (2020-10-26)]]
- [[_COMMUNITY_2.0.0-rc.8 (2020-11-2)|2.0.0-rc.8 (2020-11-2)]]
- [[_COMMUNITY_2.0.1 (2021-02-21)|2.0.1 (2021-02-21)]]
- [[_COMMUNITY_2.0.2 (2021-03-04)|2.0.2 (2021-03-04)]]
- [[_COMMUNITY_(Breaking changes) Breaking changes|(Breaking changes) Breaking changes]]
- [[_COMMUNITY_2.0.0-beta.5 (2020-10-10)|2.0.0-beta.5 (2020-10-10)]]
- [[_COMMUNITY_2.0.0-beta.7 (2020-10-12)|2.0.0-beta.7 (2020-10-12)]]
- [[_COMMUNITY_2.0.0-rc.11 (2020-11-18)|2.0.0-rc.11 (2020-11-18)]]
- [[_COMMUNITY_2.0.0-rc.16 (2021-01-12)|2.0.0-rc.16 (2021-01-12)]]
- [[_COMMUNITY_2.0.0-rc.1 (2020-10-14)|2.0.0-rc.1 (2020-10-14)]]
- [[_COMMUNITY_2.0.0-rc.2 (2020-10-17)|2.0.0-rc.2 (2020-10-17)]]
- [[_COMMUNITY_2.0.0-rc.3 (2020-10-19)|2.0.0-rc.3 (2020-10-19)]]
- [[_COMMUNITY_2.0.0-rc.4 (2020-10-21)|2.0.0-rc.4 (2020-10-21)]]
- [[_COMMUNITY_2.0.0-rc.5 (2020-10-26)|2.0.0-rc.5 (2020-10-26)]]
- [[_COMMUNITY_2.0.0-rc.8 (2020-11-2)|2.0.0-rc.8 (2020-11-2)]]
- [[_COMMUNITY_2.0.1 (2021-02-21)|2.0.1 (2021-02-21)]]
- [[_COMMUNITY_2.0.2 (2021-03-04)|2.0.2 (2021-03-04)]]
- [[_COMMUNITY_2.1.1 (2021-03-26)|2.1.1 (2021-03-26)]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY_axios.d.ts|axios.d.ts]]
- [[_COMMUNITY_module.d.ts|module.d.ts]]
- [[_COMMUNITY_AccountController.cs|AccountController.cs]]
- [[_COMMUNITY_TestModel.cs|TestModel.cs]]
- [[_COMMUNITY_SkipStockCtuInDto|SkipStockCtuInDto]]
- [[_COMMUNITY_.UpdateWarehouseAreaAsync|.UpdateWarehouseAreaAsync]]
- [[_COMMUNITY_CountInfoOfStock|CountInfoOfStock]]
- [[_COMMUNITY_Default.cshtml|Default.cshtml]]
- [[_COMMUNITY_2.0.0-rc.15(httpsgithub.comanncwbvue-vben-admincomparev2.0.0-rc.14...v2.0.0-rc.15) (2020-12-31)|[2.0.0-rc.15](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.14...v2.0.0-rc.15) (2020-12-31)]]
- [[_COMMUNITY_2.0.0-rc.16(httpsgithub.comanncwbvue-vben-admincomparev2.0.0-rc.15...v2.0.0-rc.16) (2021-01-12)|[2.0.0-rc.16](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.15...v2.0.0-rc.16) (2021-01-12)]]
- [[_COMMUNITY_2.0.0-rc.17(httpsgithub.comanncwbvue-vben-admincomparev2.0.0-rc.16...v2.0.0-rc.17) (2021-01-18)|[2.0.0-rc.17](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.16...v2.0.0-rc.17) (2021-01-18)]]
- [[_COMMUNITY_2.4.0(httpsgithub.comanncwbvue-vben-admincomparev2.2.0...v2.4.0) (2021-05-25)|[2.4.0](https://github.com/anncwb/vue-vben-admin/compare/v2.2.0...v2.4.0) (2021-05-25)]]
- [[_COMMUNITY_2.7.1(httpsgithub.comanncwbvue-vben-admincomparev2.6.1...v2.7.1) (2021-08-16)|[2.7.1](https://github.com/anncwb/vue-vben-admin/compare/v2.6.1...v2.7.1) (2021-08-16)]]
- [[_COMMUNITY_2.0.0-beta.6 (2020-10-11)|2.0.0-beta.6 (2020-10-11)]]
- [[_COMMUNITY_2.0.0-rc.16 (2020-01-12)|2.0.0-rc.16 (2020-01-12)]]
- [[_COMMUNITY_2.0.0-rc.17 (2020-01-18)|2.0.0-rc.17 (2020-01-18)]]
- [[_COMMUNITY_2.0.0-rc.18 (2021-02-05)|2.0.0-rc.18 (2021-02-05)]]
- [[_COMMUNITY_2.0.3 (2021-03-07)|2.0.3 (2021-03-07)]]
- [[_COMMUNITY_2.1.0 (2021-03-15)|2.1.0 (2021-03-15)]]
- [[_COMMUNITY_2.2.0 (2021-04-06)|2.2.0 (2021-04-06)]]
- [[_COMMUNITY_2.4.0 (2021-05-25)|2.4.0 (2021-05-25)]]
- [[_COMMUNITY_2.4.1(2021-06-01)|2.4.1(2021-06-01)]]
- [[_COMMUNITY_2.4.2(2021-06-10)|2.4.2(2021-06-10)]]
- [[_COMMUNITY_2.6.1(2021-07-19)|2.6.1(2021-07-19)]]
- [[_COMMUNITY_2.0.0-beta.56 (2020-10-11)|2.0.0-beta.56 (2020-10-11)]]
- [[_COMMUNITY_2.0.0-rc.17 (2021-01-18)|2.0.0-rc.17 (2021-01-18)]]
- [[_COMMUNITY_2.0.0-rc.18 (2021-02-05)|2.0.0-rc.18 (2021-02-05)]]
- [[_COMMUNITY_2.0.3 (2021-03-07)|2.0.3 (2021-03-07)]]
- [[_COMMUNITY_2.1.0 (2021-03-15)|2.1.0 (2021-03-15)]]
- [[_COMMUNITY_2.2.0 (2021-04-06)|2.2.0 (2021-04-06)]]
- [[_COMMUNITY_2.4.0 (2021-05-25)|2.4.0 (2021-05-25)]]
- [[_COMMUNITY_2.4.1(2021-06-01)|2.4.1(2021-06-01)]]
- [[_COMMUNITY_2.4.2(2021-06-10)|2.4.2(2021-06-10)]]
- [[_COMMUNITY_2.5.2(2021-06-27)|2.5.2(2021-06-27)]]
- [[_COMMUNITY_2.6.1(2021-07-19)|2.6.1(2021-07-19)]]
- [[_COMMUNITY_(破坏性更新) Breaking changes|(破坏性更新) Breaking changes]]
- [[_COMMUNITY__createProductionServer.ts|_createProductionServer.ts]]
- [[_COMMUNITY_types.d.ts|types.d.ts]]
- [[_COMMUNITY_WarehouseDto|WarehouseDto]]
- [[_COMMUNITY_2.0.0-beta.3 (2020-10-07)|2.0.0-beta.3 (2020-10-07)]]
- [[_COMMUNITY_2.0.0-beta.4 (2020-10-08)|2.0.0-beta.4 (2020-10-08)]]
- [[_COMMUNITY_2.5.1(2021-06-26)|2.5.1(2021-06-26)]]
- [[_COMMUNITY_2.6.0(2021-07-04)|2.6.0(2021-07-04)]]
- [[_COMMUNITY_(Breaking changes) Breaking changes|(Breaking changes) Breaking changes]]
- [[_COMMUNITY_baseModel.ts|baseModel.ts]]
- [[_COMMUNITY_env.d.ts|env.d.ts]]
- [[_COMMUNITY_useScript.ts|useScript.ts]]
- [[_COMMUNITY_index.vue|index.vue]]
- [[_COMMUNITY_Test Server|Test Server]]
- [[_COMMUNITY_vue-router.d.ts|vue-router.d.ts]]
- [[_COMMUNITY_README|README.md]]
- [[_COMMUNITY_.eslintrc.js|.eslintrc.js]]
- [[_COMMUNITY_utils.d.ts|utils.d.ts]]
- [[_COMMUNITY_WarehouseAreaAddDto|WarehouseAreaAddDto]]
- [[_COMMUNITY_WarehouseAreaUpdateDto|WarehouseAreaUpdateDto]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_StockCreateDto|StockCreateDto]]
- [[_COMMUNITY_WarehouseAreaAddDto|WarehouseAreaAddDto]]
- [[_COMMUNITY_WarehouseDto|WarehouseDto]]
- [[_COMMUNITY_FeatureProviderDto|FeatureProviderDto]]
- [[_COMMUNITY_2.0.0(httpsgithub.comanncwbvue-vben-admincomparev2.0.0-rc.18...v2.0.0) (2021-02-17)|[2.0.0](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.18...v2.0.0) (2021-02-17)]]
- [[_COMMUNITY_WarehouseInfoOfChkRsltList|WarehouseInfoOfChkRsltList]]
- [[_COMMUNITY_WmsHttpApiClientModule|WmsHttpApiClientModule]]
- [[_COMMUNITY_LanguageInfo|LanguageInfo]]
- [[_COMMUNITY_SkipStockCtuInDto|SkipStockCtuInDto]]
- [[_COMMUNITY_tsconfig-paths|tsconfig-paths]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_CountInfoOfStock|CountInfoOfStock]]
- [[_COMMUNITY_WmsEntityFrameworkCoreModule|WmsEntityFrameworkCoreModule]]
- [[_COMMUNITY_2.4.0(httpsgithub.comanncwbvue-vben-admincomparev2.2.0...v2.4.0) (2021-05-25)|[2.4.0](https://github.com/anncwb/vue-vben-admin/compare/v2.2.0...v2.4.0) (2021-05-25)]]

## God Nodes (most connected - your core abstractions)
1. `throwException()` - 257 edges
2. `isAxiosError()` - 257 edges
3. `ICGRKDAddResponseDto` - 235 edges
4. `ResponseDto` - 227 edges
5. `useI18n()` - 203 edges
6. `useDesign()` - 152 edges
7. `StockServiceProxy` - 110 edges
8. `StockService` - 95 edges
9. `t()` - 87 edges
10. `TuTa.Wms.Erp` - 85 edges

## Surprising Connections (you probably didn't know these)
- `UpdateRoleInput` --references--> `IdentityRoleUpdateDto`  [EXTRACTED]
  WMS/src/TuTa.Wms.Application.Contracts/Roles/Dtos/UpdateRoleInput.cs → WMSWeb/src/services/ServiceProxies.ts
- `UpdateUserInput` --references--> `IdentityUserUpdateDto`  [EXTRACTED]
  WMS/src/TuTa.Wms.Application.Contracts/Users/Dtos/UpdateUserInput.cs → WMSWeb/src/services/ServiceProxies.ts
- `IndexModel` --references--> `LanguageInfo`  [EXTRACTED]
  WMS/src/TuTa.Wms.AuthServer/Pages/Index.cshtml.cs → WMSWeb/src/services/ServiceProxies.ts
- `UpdateRolePermissionsInput` --references--> `UpdatePermissionsDto`  [EXTRACTED]
  WMS/src/TuTa.Wms.Application.Contracts/Roles/Dtos/UpdateRolePermissionsInput.cs → WMSWeb/src/services/ServiceProxies.ts
- `useMenuSearch()` --indirect_call--> `handleClose()`  [INFERRED]
  WMSWeb/src/components/Application/src/search/useMenuSearch.ts → WMSWeb/src/components/Application/src/search/AppSearchModal.vue

## Import Cycles
- 1-file cycle: `WMSWeb/src/views/admin/roles/AbpRole.vue -> WMSWeb/src/views/admin/roles/AbpRole.vue`
- 1-file cycle: `WMSWeb/src/views/task/agv/AgvTaskManagement.vue -> WMSWeb/src/views/task/agv/AgvTaskManagement.vue`
- 1-file cycle: `WMSWeb/src/views/task/historys/IncellHis.vue -> WMSWeb/src/views/task/historys/IncellHis.vue`
- 1-file cycle: `WMSWeb/src/views/task/historys/OutcellHis.vue -> WMSWeb/src/views/task/historys/OutcellHis.vue`
- 1-file cycle: `WMSWeb/src/views/task/tasks/task.vue -> WMSWeb/src/views/task/tasks/task.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/boxs/StorageBox.vue -> WMSWeb/src/views/warehouse/boxs/StorageBox.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/cells/Cell.vue -> WMSWeb/src/views/warehouse/cells/Cell.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/goods/Good.vue -> WMSWeb/src/views/warehouse/goods/Good.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/goods/linliaoOrder.vue -> WMSWeb/src/views/warehouse/goods/linliaoOrder.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/goods/StockAdjustment.vue -> WMSWeb/src/views/warehouse/goods/StockAdjustment.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/goods/BarcodeList.vue -> WMSWeb/src/views/warehouse/goods/BarcodeList.vue`
- 1-file cycle: `WMSWeb/src/views/admin/users/AbpUser.vue -> WMSWeb/src/views/admin/users/AbpUser.vue`
- 1-file cycle: `WMSWeb/src/views/mobile/views/Stock.vue -> WMSWeb/src/views/mobile/views/Stock.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/materialBasic/MaterialBasic.vue -> WMSWeb/src/views/warehouse/materialBasic/MaterialBasic.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/outstock/Outstock.vue -> WMSWeb/src/views/warehouse/outstock/Outstock.vue`
- 1-file cycle: `WMSWeb/src/views/warehouse/wares/Ware.vue -> WMSWeb/src/views/warehouse/wares/Ware.vue`
- 1-file cycle: `WMSWeb/src/components/Preview/src/functional.ts -> WMSWeb/src/components/Preview/src/functional.ts`
- 1-file cycle: `WMSWeb/src/views/erp/AsnList.vue -> WMSWeb/src/views/erp/AsnList.vue`
- 1-file cycle: `WMSWeb/src/views/task/statistics/MaterialStockStatistics.vue -> WMSWeb/src/views/task/statistics/MaterialStockStatistics.vue`
- 1-file cycle: `WMSWeb/src/views/task/statistics/WorkshopReceiptStatistics.vue -> WMSWeb/src/views/task/statistics/WorkshopReceiptStatistics.vue`

## Communities (606 total, 148 thin omitted)

### Community 0 - "ErpWorkstationMaterialRequest"
Cohesion: 0.02
Nodes (25): TuTa.Wms.Application.Erp, Wms.ConfigTool, TuTa.Wms.Application.Contracts.Erp, TuTa.Wms.Application.Contracts.Erp.IDto, TuTa.Wms.Migrations, TuTa.Wms.BarcodeLists.ValueObjects, Wms.RedisTool, TuTa.Wms.Erp.Entities (+17 more)

### Community 1 - "IRepository"
Cohesion: 0.05
Nodes (21): TuTa.Wms.EntityFrameworkCore.Repositories, TuTa.Wms.Erp.Repositories, TuTa.Wms.Erp.Dto, TuTa.Wms.Erp.Aggregates, TuTa.Wms.Erp.IDto, TuTa.Wms.HttpApi.Controllers.Erp, TuTa.Wms.EntityFrameworkCore.Repositories.Erp, TuTa.Wms.Erp (+13 more)

### Community 2 - "CellDto"
Cohesion: 0.06
Nodes (49): TuTa.Wms.StockInHistories.Aggregates, TuTa.Wms.Warehouses.Aggregates, TuTa.Wms.ChkResultLists.Events, TuTa.Wms.Stocks.Aggregates, TuTa.Wms.StockOutHistories.Aggregates, TuTa.Wms.BarcodeChecks.Aggregates, TuTa.Wms.Materials, TuTa.Wms.Cells.Entities (+41 more)

### Community 3 - "ErpWorkshopMaterialTransfer"
Cohesion: 0.06
Nodes (27): ResponseDto, SkipInDto, Guid, CellWithMaterialDto, PagedStockMoveQueryDto, DateTime, StockInType, StockStatus (+19 more)

### Community 4 - "Cell"
Cohesion: 0.12
Nodes (17): Guid, BoxBindCellEvent, Guid, BoxDisBindCellEvent, CellManager, ICellRepository, ILogger, object (+9 more)

### Community 5 - "ErpWorkstationMaterialReceiptDto"
Cohesion: 0.04
Nodes (31): AbpApplicationCreationOptions, AbpControllerBase, AbpIntegratedTest, TuTa.Wms.Localization, Wms, TuTa.Wms, TuTa.Wms.MultiTenancy, TuTa.Wms.Web (+23 more)

### Community 6 - "ErpMaterial"
Cohesion: 0.02
Nodes (49): ApiException, CGRKDParams, EnumCheckResult, EnumCheckType, ErpDeliveryStationResponseDto, ExtensionPropertyApiCreateDto, ExtensionPropertyApiGetDto, ExtensionPropertyApiUpdateDto (+41 more)

### Community 7 - "IPickListService"
Cohesion: 0.02
Nodes (43): CGRKDAddResponseDto, ErpAsnListResponseDto, ErpDeliveryOrderCreateDto, ErpDeliveryOrderDto, ErpInboundOrderDto, ErpInboundOrderRequestDto, ErpOutboundOrderRequestDto, GetChkByBarcodeDto (+35 more)

### Community 8 - "StockService"
Cohesion: 0.04
Nodes (90): GetUserInfoByUserIdModel, GetUserInfoByUserIdParams, GetUserInfoModel, LoginParams, LoginResultModel, Api, getAbpApplicationConfiguration(), getPermCodeByUserId() (+82 more)

### Community 9 - "AgvTaskManager"
Cohesion: 0.07
Nodes (21): DateTime, StockCreateDto, Dictionary, Guid, IdentityUserManager, IEnumerable, ILogger, IRepository (+13 more)

### Community 10 - ".FindByIdAsync"
Cohesion: 0.03
Nodes (3): ERP_ASNServiceProxy, StockServiceProxy, throwException()

### Community 11 - "SelectCall.vue"
Cohesion: 0.05
Nodes (82): setup(), setup(), Params, renderEditCell(), setup(), Options, setup(), State (+74 more)

### Community 12 - "CreateOutboundTaskFromBarcode.vue"
Cohesion: 0.04
Nodes (48): axios, Api, Api, getMenuList(), getMenuListResultModel, RouteItem, { uploadUrl = '' }, { getAntdLocale } (+40 more)

### Community 13 - "Erp_StockEventHandler"
Cohesion: 0.05
Nodes (45): DateTime, Guid, ErpWorkstationMaterialRequestDto, ErpWorkstationMaterialRequestQueryDto, ErpWorkstationMaterialRequestRequestDto, ErpWorkstationMaterialRequestResponseDto, ErpWorkstationMaterialRequestStatusUpdateDto, Guid (+37 more)

### Community 14 - "BoxService"
Cohesion: 0.12
Nodes (17): { createConfirm }, createOutStockTask(), getStocks(), [openFullLoading, closeFullLoading], [registerOutstockModal, { openModal: openOutstockModal }], [registerTable, { getDataSource, reload, getSelectRows, clearSelectedRowKeys }], searchFormSchema, _StockServiceProxy (+9 more)

### Community 16 - "ValueObject"
Cohesion: 0.05
Nodes (70): setup(), props, setup(), useCountdown(), apiFunParams, props, setup(), props (+62 more)

### Community 17 - "IdInput"
Cohesion: 0.03
Nodes (66): IRepository, PickOrderStatus, ILogger, List, Task, DepartmentService, ILogger, List (+58 more)

### Community 18 - "SkipService"
Cohesion: 0.03
Nodes (20): ApplicationApiDescriptionModel, ApplicationLocalizationConfigurationDto, ControllerApiDescriptionModel, ErpMaterialResponseDto, ExtensionPropertyApiDto, ExtensionPropertyUiDto, IApplicationApiDescriptionModel, IApplicationLocalizationConfigurationDto (+12 more)

### Community 19 - "StockService.cs"
Cohesion: 0.03
Nodes (60): CellAddDto, ICellAddDto, emit, [registerCellForm, { getFieldsValue, validate, resetFields, }], [registerModal, { changeOkLoading, closeModal }], submit(), { t }, emit (+52 more)

### Community 20 - "MaterialSampling.vue"
Cohesion: 0.06
Nodes (33): List, GetByBarcodeBoxDto, Guid, NoPlanPickListDelDto, Guid, NoPlanPickListEditDto, Guid, NoPlanPickOutCreateDto (+25 more)

### Community 21 - "ISkipService"
Cohesion: 0.05
Nodes (40): boxCode, { createConfirm }, deleteStock(), diskcancel(), fetchLaneCellStatus(), focus1, focus2, goodheight (+32 more)

### Community 22 - "PickListService"
Cohesion: 0.21
Nodes (9): QrCode, getFormTitle, { getLoginState }, { t }, getShow, { handleBackLogin, getLoginState }, { t }, LoginStateEnum (+1 more)

### Community 23 - "TuTa.Wms.Application.Contracts.Shared"
Cohesion: 0.04
Nodes (67): EXCEPTION_COMPONENT(), getParentLayout(), LAYOUT(), addToChildren(), asyncImportRoute(), dynamicImport(), flatMultiLevelRoutes(), IFRAME() (+59 more)

### Community 24 - "TuTa.Wms.Erp.Entities"
Cohesion: 0.06
Nodes (33): PagedResultDto, BoxManager, IBoxRepository, ILogger, object, UnitOfWorkManager, CellEventHandler, CellRunStatus (+25 more)

### Community 25 - "BoxBind.vue"
Cohesion: 0.07
Nodes (30): CanCreate, DispatchToRcs, FullAuditedAggregateRoot, ILogger, IRepository, IServiceProvider, Task, UnitOfWork (+22 more)

### Community 26 - "Autos.vue"
Cohesion: 0.06
Nodes (40): ControllerBase, DateTime, Guid, List, ErpDeliveryOrderCreateDto, ErpDeliveryOrderDto, ErpDeliveryOrderItemCreateDto, ErpDeliveryOrderItemDto (+32 more)

### Community 27 - "TuTa.Wms.Erp"
Cohesion: 0.07
Nodes (47): ButtonProps, setup(), add(), del(), ButtonOptions, setup(), createPlaceholderMessage(), DATE_TYPE (+39 more)

### Community 28 - "PickList"
Cohesion: 0.03
Nodes (71): devDependencies, autoprefixer, commitizen, @commitlint/cli, @commitlint/config-conventional, conventional-changelog-cli, cross-env, dotenv (+63 more)

### Community 29 - "AgvTaskPagedQueryDto"
Cohesion: 0.04
Nodes (61): AppProviderContextProps, key, appStore, debounceRefresh, el, emit, init(), props (+53 more)

### Community 30 - "Department"
Cohesion: 0.05
Nodes (4): RoleServiceProxy, RolesServiceProxy, UserServiceProxy, UsersServiceProxy

### Community 31 - "CreateOutStockTaskSummary.vue"
Cohesion: 0.09
Nodes (23): ErpMaterialRequestDto, ErpMaterialResponseDto, Guid, ILogger, List, Task, UnitOfWork, ErpMaterialAppService (+15 more)

### Community 32 - "ErpDeliveryStation"
Cohesion: 0.06
Nodes (51): RoleInfo, props, setup(), createAppProviderContext(), setup(), setup(), setup(), setup() (+43 more)

### Community 33 - "ErpDbContext"
Cohesion: 0.07
Nodes (26): CellAddDto, CellLaneStatusDto, Guid, List, CellsBindAreaDto, Guid, List, CellsDisBindFromAreaDto (+18 more)

### Community 34 - "RolePermissionAppService"
Cohesion: 0.04
Nodes (12): AbpApiDefinitionServiceProxy, AbpApplicationConfigurationServiceProxy, AbpApplicationLocalizationServiceProxy, BarcodeListServiceProxy, DepartmentServiceProxy, DynamicClaimsServiceProxy, ErpMidTableServiceProxy, LoginServiceProxy (+4 more)

### Community 35 - "WmsDbContext.cs"
Cohesion: 0.14
Nodes (14): Guid, ErpWorkshopMaterialTransfer, MaterialTransferStatus, Guid, Task, ErpWorkshopMaterialTransferManager, Guid, List (+6 more)

### Community 36 - "TuTa.Wms.AgvTasks"
Cohesion: 0.05
Nodes (80): { prefixCls }, { t }, useAppProviderContext(), getClass, { prefixCls }, props, getIcons(), getSvgIcons() (+72 more)

### Community 37 - "IRoleAppService"
Cohesion: 0.11
Nodes (11): PickItemStatus, Guid, List, PickItem, DateTime, Guid, PickStock, CancellationToken (+3 more)

### Community 38 - "IApplicationService"
Cohesion: 0.09
Nodes (40): getClass, { getDarkMode, setDarkMode, getShowDarkModeToggle }, isDark, { prefixCls }, toggleDarkMode(), setup(), setup(), ThemeEnum (+32 more)

### Community 39 - "ErpOutboundOrderDto"
Cohesion: 0.07
Nodes (3): ErpOutboundOrderServiceProxy, ErpWorkstationMaterialRequestServiceProxy, isAxiosError()

### Community 40 - "ErpAsnAppService"
Cohesion: 0.04
Nodes (47): AbpDbContext, Entity, IDesignTimeDbContextFactory, RecheckItemStatus, object, ErpDeptType, ErpDeptTypeDetail, DateTime (+39 more)

### Community 41 - "StockConsolidationPlanner"
Cohesion: 0.08
Nodes (27): ILocalEventBus, ILogger, PagedResultDto, Task, UnitOfWork, SkipService, CancellationToken, Guid (+19 more)

### Community 42 - "ErpPickOrderSyncJob"
Cohesion: 0.08
Nodes (47): createSearchReg(), SearchResult, transform(), useMenuSearch(), setup(), MenuState, useOpenKeys(), RouteLocationRawEx (+39 more)

### Community 43 - "ErpAsn"
Cohesion: 0.04
Nodes (19): ApplicationAuthConfigurationDto, ApplicationConfigurationDto, ApplicationFeatureConfigurationDto, ApplicationGlobalFeatureConfigurationDto, ApplicationSettingConfigurationDto, ClockDto, CurrentTenantDto, CurrentUserDto (+11 more)

### Community 44 - "MaterialService"
Cohesion: 0.05
Nodes (26): cacheCipher, createLocalStorage(), createOptions(), createSessionStorage(), createStorage(), Options, WebStorage, Cache (+18 more)

### Community 45 - "EnumCheckType"
Cohesion: 0.11
Nodes (22): BasicDragVerify, RotateDragVerify, setup(), basicProps, rotateProps, { t }, DragVerifyActionType, MoveData (+14 more)

### Community 46 - "PagedStockQueryDto"
Cohesion: 0.06
Nodes (45): emit, getValue, handleValueChange(), MODE, props, props, setup(), ImageItem (+37 more)

### Community 47 - "WarehouseAreaDto"
Cohesion: 0.08
Nodes (25): InspectionStatus, RunStatus, StockDirectCreateDto, List, QueryDataInPage, StockInType, StockInTypeHelper, StockStatus (+17 more)

### Community 48 - "ErpPickMan"
Cohesion: 0.09
Nodes (25): BoxStatus, Guid, ILogger, List, PagedResultDto, Task, BoxService, List (+17 more)

### Community 49 - "StockConsolidationWorker"
Cohesion: 0.09
Nodes (19): boxCode, BoxSelectModal, CellSelectModal, endcellCode, eventFnboxselect(), getByStock(), goodheight, GoodsDetail (+11 more)

### Community 50 - "WmsController"
Cohesion: 0.05
Nodes (33): ILogger, IRepository, LocalEventBus, object, Task, UnitOfWorkManager, Erp_StockEventHandler, ILogger (+25 more)

### Community 51 - "IRecheckListService"
Cohesion: 0.42
Nodes (5): CancellationToken, Guid, List, Task, EfCorePickListRepository

### Community 52 - "index.vue"
Cohesion: 0.15
Nodes (16): setup(), dataTransfer, visibleData, useFullScreen(), UseFullScreenContext, basicProps, modalProps, { t } (+8 more)

### Community 53 - "IWarehouseService"
Cohesion: 0.17
Nodes (17): ScrollContainerOptions, BasicDrawer, DrawerActionType, DrawerFooterProps, DrawerInstance, DrawerProps, RegisterFn, ReturnInnerMethods (+9 more)

### Community 54 - "OpenIddictDataSeedContributor"
Cohesion: 0.12
Nodes (16): BoxAddDto, Guid, BoxDto, BoxUpdateDto, PagedBoxesQueryDto, Guid, List, PagedResultDto (+8 more)

### Community 55 - "RecheckItem"
Cohesion: 0.12
Nodes (21): emit, [registerForm, { getFieldsValue, validate, setFieldsValue, resetFields }], [registerModal, { changeOkLoading, closeModal }], submit(), { t }, buildCreateDto(), buildUpdateDto(), createMaterialAsync() (+13 more)

### Community 56 - "IStockRepository"
Cohesion: 0.09
Nodes (18): add(), detailcolumns, ordercolumns, cancel(), dataSource, emit, good, goodlist (+10 more)

### Community 57 - "ErpInboundOrder"
Cohesion: 0.05
Nodes (43): Button, PopConfirmButton, attrs, getBindValue, getButtonClass, props, props, setup() (+35 more)

### Community 58 - "SessionTimeoutLogin.vue"
Cohesion: 0.13
Nodes (14): RecheckListStatus, DateTime, Guid, List, RecheckList, DateTime, Task, IEnumerable (+6 more)

### Community 59 - "StockConsolidationService"
Cohesion: 0.11
Nodes (14): { createConfirm }, findtype, fliter, focus1, goodheight, goodsCode, GoodsDetail, [registerGoodsDetailModal, { openModal: openGoodsDetailModal }] (+6 more)

### Community 60 - "BoxEventHandler"
Cohesion: 0.22
Nodes (10): ILogger, List, Task, RecheckListService, CancellationToken, Guid, List, Task (+2 more)

### Community 61 - "AgvTaskService"
Cohesion: 0.08
Nodes (33): go, goHome(), registerGlobComp(), setupGlobDirectives(), loadingDirective, setupLoadingDirective(), authDirective, isAuth() (+25 more)

### Community 63 - "NewAccountController"
Cohesion: 0.06
Nodes (6): activeKey, getUserInfo, recheckcount, userStore, validTabs, viewStore

### Community 64 - "IMaterialService"
Cohesion: 0.08
Nodes (21): { createConfirm }, getUserInfo, handleLoginOut(), userStore, handleUnbind(), inputRef, loading, resolveCellCode() (+13 more)

### Community 65 - "PagingBase"
Cohesion: 0.14
Nodes (11): columns, data, dataSource, date, emit, getdata(), hiscolumns, innerColumns (+3 more)

### Community 66 - "WmsDbContext"
Cohesion: 0.11
Nodes (16): CancellationToken, ILogger, Task, UnitOfWorkManager, ErpBarcodeSyncJob, DateTime, object, ErpBarcode (+8 more)

### Community 67 - "Header.vue"
Cohesion: 0.06
Nodes (27): AbpUnitOfWorkOptions, TuTa.Wms.EntityFrameworkCore.Domains, TuTa.Wms.Samples, TuTa.Wms.EntityFrameworkCore.Applications, TuTa.Wms.EntityFrameworkCore.Samples, Fact, IIdentityUserAppService, Task (+19 more)

### Community 68 - "StockInHistory_StockEventHandler"
Cohesion: 0.10
Nodes (17): cellStatusSelectItem, { createConfirm }, createFormSchema, _DepartmentServiceProxy, editFormSchema, _ErpMidTableServiceProxy, [openFullLoading, closeFullLoading], _PickNotifierServiceProxy (+9 more)

### Community 69 - "RecheckList"
Cohesion: 0.13
Nodes (18): Guid, ErpWorkshopMaterialTransferDto, ErpWorkshopMaterialTransferRequestDto, ErpWorkshopMaterialTransferResponseDto, ErpWorkshopMaterialTransferStatusUpdateDto, Guid, List, PagedResultDto (+10 more)

### Community 70 - "IErpAsnAppService"
Cohesion: 0.13
Nodes (14): PagedSkipDto, SkipAddDto, SkipDto, SkipOutDto, PagedResultDto, Task, ISkipService, HttpGet (+6 more)

### Community 71 - "ErpRecheckNotifierSyncJob"
Cohesion: 0.13
Nodes (24): props, createContextMenu(), destroyContextMenu(), menuManager, Axis, ContextMenuItem, ContextMenuProps, CreateContextOptions (+16 more)

### Community 72 - "ErpPickOrder"
Cohesion: 0.07
Nodes (30): allCellsAndMaterialAreaGet(), cellChartFormSchema, _CellServiceProxy, cellStatusSelectItem, cellTypeSelectItem, { createConfirm }, createFormSchema, defaultHeader() (+22 more)

### Community 73 - "IWarehouseRepository"
Cohesion: 0.14
Nodes (14): Guid, IdInput, ListResultDto, PagedResultDto, Task, IUserAppService, HttpPost, ListResultDto (+6 more)

### Community 74 - "TuTa.Wms.Materials.Aggregates"
Cohesion: 0.06
Nodes (25): baoshu, barcodeGet(), boxCode, cell, cellCode, CellSelectModal, columns, createLiftIn() (+17 more)

### Community 75 - "TuTa.Wms.AgvTasks.Dto"
Cohesion: 0.08
Nodes (32): allBoxesGet(), boxTypeSelectItem, { createConfirm }, createManyStorageBoxAsync(), defaultHeader(), deleteStorageBoxAsync(), deleteStorageBoxDetailAsync(), editFormSchema (+24 more)

### Community 76 - "ErpOutboundOrderAppService"
Cohesion: 0.09
Nodes (10): cellcolumns, celldataSource, _CellServiceProxy, _DepartmentServiceProxy, hiscolumns, _OutboundListServiceProxy, outcolumns, _PickListServiceProxy (+2 more)

### Community 77 - "WarehouseService"
Cohesion: 0.06
Nodes (12): ActionApiDescriptionModel, ControllerInterfaceApiDescriptionModel, IActionApiDescriptionModel, IControllerInterfaceApiDescriptionModel, IInterfaceMethodApiDescriptionModel, IMethodParameterApiDescriptionModel, InterfaceMethodApiDescriptionModel, IParameterApiDescriptionModel (+4 more)

### Community 78 - "ErpRecheckNotifier"
Cohesion: 0.06
Nodes (30): IdentityClaimType, IdentityLinkUser, IdentityRole, IdentitySecurityLog, IdentityUserDelegation, IIdentityDbContext, ITenantManagementDbContext, OrganizationUnit (+22 more)

### Community 79 - "ErpStockAftChk"
Cohesion: 0.11
Nodes (8): FileController, UserController, router, filePath, UserService, fakeUserInfo, UserService, Result

### Community 81 - "EfCoreBoxRepository"
Cohesion: 0.07
Nodes (35): asnCode, asnDataList, asnInputRef, boxCode, confirmNotQualified(), confirmQualified(), { createConfirm }, deleteStock() (+27 more)

### Community 82 - "TuTa.Wms.Stocks"
Cohesion: 0.07
Nodes (17): TuTa.Wms.Controllers.Stocks, TuTa.Wms.Controllers.Skips, TuTa.Wms.Departments.Aggregates, TuTa.Wms.StockConsolidations, TuTa.Wms.Cells.Dtos, TuTa.Wms.Controllers.Warehouses, TuTa.Wms.Application.Contracts.Shared, TuTa.Wms.Skips.Dtos (+9 more)

### Community 83 - "TuTa.Wms.Users.Dtos"
Cohesion: 0.09
Nodes (15): TuTa.Wms.Controllers.PickLists, TuTa.Wms.Erp.Dtos, TuTa.Wms.PickLists.Dtos, TuTa.Wms.Controllers.Erp, ErpGoodsAftChkDto, ErpPickManDto, List, Task (+7 more)

### Community 84 - "TuTa.Wms.StockInHistories.ValueObjects"
Cohesion: 0.08
Nodes (15): Migration, MigrationBuilder, AddStockFields, MigrationBuilder, AddErpAsn, MigrationBuilder, UpdateErpAsnFields, MigrationBuilder (+7 more)

### Community 85 - "EfCoreRepository"
Cohesion: 0.12
Nodes (25): UploadApiResult, useModalContext(), setup(), createActionColumn(), createPreviewActionColumn(), createPreviewColumns(), createTableColumns(), { t } (+17 more)

### Community 86 - "UserAppService"
Cohesion: 0.11
Nodes (16): batchAutoAllocateStockDetailWithTotal(), columns4, DataItem, dataSource, editableData, emit, getdata(), good (+8 more)

### Community 87 - "ErpBarcode"
Cohesion: 0.06
Nodes (31): createLoading(), LoadingProps, Fn, useLoading(), UseLoadingOptions, SizeEnum, sizeMap, SizeNumberEnum (+23 more)

### Community 88 - "Ware.vue"
Cohesion: 0.15
Nodes (13): barcodeGet(), boxCode, CellSelectModal, clear(), createAgv(), { createConfirm }, goods, newBoxCode (+5 more)

### Community 89 - "ErpStateChgNotifier"
Cohesion: 0.07
Nodes (26): boxCode, BoxModal, { createConfirm }, getByBarcodeBoxCode(), getUserInfo, good, goodheight, goodsCode (+18 more)

### Community 90 - "TuTa.Wms.RecheckLists"
Cohesion: 0.07
Nodes (24): boxCode, BoxModal, { createConfirm }, getByBarcodeBoxCode(), getUserInfo, goodheight, goodsCode, GoodsDetail (+16 more)

### Community 91 - "TuTa.Wms.StockConsolidations"
Cohesion: 0.05
Nodes (39): addStock(), canBatchOut, cellCode, cellInputRef, displayCellCode, executeBatchOut(), executing, loading (+31 more)

### Community 92 - "WmsDbMigrationService"
Cohesion: 0.07
Nodes (28): allDepartmentsGet(), columns, pagedPickItemsGet(), batchNo, count, dapdata, data(), dataSource (+20 more)

### Community 93 - ".GetPagedMaterialsAsync"
Cohesion: 0.09
Nodes (20): Consumes, IFormFile, NonAction, MaterialCreateDto, List, MaterialImportResultDto, MaterialUpdateDto, PagedMaterialsQueryDto (+12 more)

### Community 94 - "ErpBarcodeSyncJob"
Cohesion: 0.10
Nodes (18): TuTa.Wms.StockOutHistories.Dtos, TuTa.Wms.Controllers.StockOutHistories, DateTime, PagedStockOutHistoryQueryDto, DateTime, StockOutHistoryDto, PagedResultDto, Task (+10 more)

### Community 95 - "ErpMove"
Cohesion: 0.07
Nodes (30): asnCode, asnCodeValidated, asnInputRef, asnOrderColumns, AsnOrderGroup, asnOrderGroups, AsnOrderItem, boxCode (+22 more)

### Community 96 - "AuditedEntityDto"
Cohesion: 0.06
Nodes (30): 2.0.0 (2021-02-18), 2.0.0-beta.1(2020-09-30), 2.0.0-beta.2 (2020-10-07), 2.0.0-beta.3 (2020-10-07), 2.0.0-beta.4 (2020-10-08), 2.0.0-rc.12 (2020-11-30), 2.0.0-rc.13 (2020-12-10), 2.3.0 (2021-04-10) (+22 more)

### Community 97 - "ITransientDependency"
Cohesion: 0.15
Nodes (14): ValueObject, DateTime, Guid, BarcodeList, DateTime, Task, IEnumerable, CountInfoOfBarcodeList (+6 more)

### Community 98 - "Stock"
Cohesion: 0.16
Nodes (7): Guid, Guid, List, WarehouseType, Warehouse, Guid, WarehouseArea

### Community 99 - "ErpInboundOrderDto"
Cohesion: 0.07
Nodes (29): dependencies, ali-oss, ant-design-vue, crypto-js, echarts, @iconify/iconify, jwt-decode, @kjgl77/datav-vue3 (+21 more)

### Community 100 - "ErpInboundOrderAppService"
Cohesion: 0.04
Nodes (58): { changeLocale, getLocale }, getLocaleText, handleMenuEvent(), props, selectedKeys, toggleLocale(), Dropdown, setup() (+50 more)

### Community 101 - "AGVIncell.vue"
Cohesion: 0.07
Nodes (14): ErpAsnDto, ErpAsnSaveResponseDto, ErpAsnValidateResponseDto, IErpAsnDto, IErpAsnSaveResponseDto, IErpAsnValidateResponseDto, AsnGroupRow, asnCode (+6 more)

### Community 103 - "Material"
Cohesion: 0.12
Nodes (15): batchAutoAllocateStockDetail(), DataItem, dataSource, editableData, emit, getdata(), good, handleTableChange() (+7 more)

### Community 104 - "EfCoreSkipRepository"
Cohesion: 0.20
Nodes (8): Guid, List, ErpAsnListResponseDto, ErpAsnSaveResponseDto, ErpAsnValidateResponseDto, PuArrVouchAddResponseDto, Task, IErpAsnAppService

### Community 105 - "ChkResultListEventHandler"
Cohesion: 0.14
Nodes (15): PagingRoleListInput, Guid, UpdateRoleInput, ListResultDto, PagedResultDto, Task, IRoleAppService, HttpPost (+7 more)

### Community 106 - "IStockConsolidationService"
Cohesion: 0.08
Nodes (21): barcodeGet(), boxCode, cellCode, columns, { createConfirm }, getUserInfo, goodheight, goodsCode (+13 more)

### Community 107 - "RoleAppService"
Cohesion: 0.10
Nodes (19): AuditedEntityDto, TuTa.Wms.StockInHistories, TuTa.Wms.Controllers.StockInHistories, TuTa.Wms.StockInHistories.Dtos, DateTime, PagedStockInHistoryQueryDto, DateTime, StockInHistoryDto (+11 more)

### Community 108 - ".GetPagedStockInHistoriesAsync"
Cohesion: 0.10
Nodes (6): ConnectionMultiplexer, Action, int, KeyValuePair, Task, RedisClientByStaEx

### Community 109 - "ErpDeptType"
Cohesion: 0.13
Nodes (16): IApplicationService, AgvCallBackRequest, DateTime, AgvTaskPagedQueryDto, List, AgvTaskPagedResultDto, ResultAgvTaskDto, Task (+8 more)

### Community 110 - "AuditedAggregateRoot"
Cohesion: 0.03
Nodes (54): Authority, BasicArrow, BasicHelp, BasicTitle, props, setup(), getClass, { prefixCls } (+46 more)

### Community 111 - "PuArrVouchAddRequestDto"
Cohesion: 0.08
Nodes (25): actualOutboundQuantity, allStocks, barcode, barcodeInputRef, canCreate, createOutboundTask(), customRow(), executing (+17 more)

### Community 112 - "StockEventHandler"
Cohesion: 0.10
Nodes (24): AbpController, Guid, ErpWorkstationMaterialReceiptDto, DateTime, Guid, List, PagedResultDto, Task (+16 more)

### Community 113 - "home.vue"
Cohesion: 0.08
Nodes (24): AuditedAggregateRoot, EfCoreRepository, Guid, BarcodeCheck, DateTime, Guid, Move, CancellationToken (+16 more)

### Community 114 - "ErpNoPlanPickType"
Cohesion: 0.13
Nodes (16): IPermissionAppService, GetPermissionInput, List, PermissionOutput, PermissionTreeDto, UpdateRolePermissionsInput, Task, IRolePermissionAppService (+8 more)

### Community 115 - "Stock_RecheckListEventHandler"
Cohesion: 0.18
Nodes (8): JToken, ErpAsnDto, ErpLoginResponseDto, ILogger, List, Task, ErpAsnAppService, ErpSettings

### Community 116 - "ApplicationService"
Cohesion: 0.21
Nodes (9): Method, RestResponse, Dictionary, Task, HttpApiHelper, ResultAgvTaskDto, List, Task (+1 more)

### Community 117 - "BoxDiskWithAsn.vue"
Cohesion: 0.07
Nodes (26): Task, scripts, bootstrap, build, build:no-cache, build:test, clean:cache, clean:lib (+18 more)

### Community 118 - "StockOutHistory"
Cohesion: 0.07
Nodes (26): IPagedStockQueryDto, PagedStockQueryDto, scancellCode(), findtype, fliter, focus1, generateSummary(), outCellCode (+18 more)

### Community 119 - "ErpInboundOrderController"
Cohesion: 0.18
Nodes (8): BoxAftChkStatus, ChkResultListStatus, Guid, List, ChkResultList, Guid, ChkResultBox, WmsDomainHelper

### Community 120 - "StockConsolidationModels.cs"
Cohesion: 0.12
Nodes (17): DomainService, DateTime, Guid, ErpWorkstationMaterialReceipt, DateTime, Task, ErpWorkstationMaterialReceiptManager, DateTime (+9 more)

### Community 121 - "TuTa.Wms"
Cohesion: 0.06
Nodes (35): ILogger, PagedResultDto, Task, ChkResultListService, List, GetChkByBarcodeDto, DateTime, StockInType (+27 more)

### Community 122 - "ILocalEventHandler"
Cohesion: 0.12
Nodes (16): TuTa.Wms.Controllers.RecheckLists, TuTa.Wms.RecheckLists.Dtos, DateTime, RecheckItemDto, RecheckItemQueryDto, RecheckPickOutDto, RecheckStockDto, List (+8 more)

### Community 123 - "BarcodeDto"
Cohesion: 0.13
Nodes (16): userInfo, demoList, authRoute, backRoute, dashboardRoute, levelRoute, linkRoute, sysRoute (+8 more)

### Community 125 - "TuTa.Wms.Application"
Cohesion: 0.08
Nodes (20): createOutStockTask(), findtype, fliter, focus1, goodheight, goodsCode, hiddenCount, outCellCode (+12 more)

### Community 126 - "TuTa.Wms.EntityFrameworkCore"
Cohesion: 0.13
Nodes (8): DateTime, Guid, AsnStatus, ErpAsn, Guid, List, Task, ErpAsnRepository

### Community 127 - "TuTa.Wms.Domain.Shared"
Cohesion: 0.04
Nodes (67): ExpExcelModal, /@/components/Excel/src/Export2Excel, aoaToSheetXlsx(), jsonToSheetXlsx(), AoAToSheet, ExportModalResult, JsonToSheet, AgvTaskStatus (+59 more)

### Community 128 - "GoodsBind.vue"
Cohesion: 0.10
Nodes (6): ImgState, props, setup(), StatueEnum, NotifyApi, VAxios

### Community 129 - "IErpAsnRepository"
Cohesion: 0.10
Nodes (23): canOut, cellCode, cellInputRef, customRow(), displayCellCode, executeOut(), executing, finishCheck() (+15 more)

### Community 130 - "IErpInboundOrderRepository"
Cohesion: 0.04
Nodes (72): useDrawer(), useModal(), EditRecordRow, useTable(), ConfirmOptions, createConfirm(), createErrorModal(), createInfoModal() (+64 more)

### Community 131 - "IErpOutboundOrderRepository"
Cohesion: 0.05
Nodes (36): autoAllocateStockDetail(), autocolumns, columns2, stockDetailByCheckNoAreaPaged(), count, dataSource, emit, getdata() (+28 more)

### Community 132 - ".GetByBoxAsync"
Cohesion: 0.09
Nodes (22): compilerOptions, allowJs, allowSyntheticDefaultImports, baseUrl, esModuleInterop, experimentalDecorators, forceConsistentCasingInFileNames, jsx (+14 more)

### Community 133 - "ErpWarehouseAreaPrdt"
Cohesion: 0.08
Nodes (28): DataFormatter, ICell, IRow, MaterialBasicImportRow, ILogger, Task, BarcodeListService, Guid (+20 more)

### Community 134 - ".Configure"
Cohesion: 0.16
Nodes (12): Guid, ChkResultListDto, PagedCheckItemQueryDto, PagedResultDto, Task, IChkResultListService, HttpGet, HttpPost (+4 more)

### Community 135 - "TuTa.Wms.HttpApi"
Cohesion: 0.14
Nodes (8): ServiceConfigurationContext, WmsDomainSharedModule, OneTimeRunner, WmsGlobalFeatureConfigurator, OneTimeRunner, WmsModuleExtensionConfigurator, OneTimeRunner, WmsEfCoreEntityExtensionMappings

### Community 136 - "PickListEventHandler"
Cohesion: 0.11
Nodes (16): CancellationToken, ILogger, Task, UnitOfWorkManager, ErpStateChgSyncJob, DateTime, object, ErpStateChgNotifier (+8 more)

### Community 137 - "EfCoreWarehouseRepository"
Cohesion: 0.08
Nodes (23): 1. 新增文件, 1. 组盘取消弹窗, 2. 修改文件, 2. 数据表格, 3. 操作按钮, 3. 组件特性, 使用流程, 兼容性说明 (+15 more)

### Community 138 - "StockInHistoryService"
Cohesion: 0.09
Nodes (19): columns, count, dapdata, data(), dataSource, date1, date2, findtype (+11 more)

### Community 139 - "StockOutHistoryService"
Cohesion: 0.16
Nodes (10): TuTa.Wms.Shared, TuTa.Wms.Roles.Dtos, TuTa.Wms.Controllers.Systems, TuTa.Wms.Roles, IEnumerable, int, ValidationContext, ValidationResult (+2 more)

### Community 140 - "TuTa.Wms.Domain"
Cohesion: 0.07
Nodes (33): Guid, ErpDeliveryStationDto, ErpDeliveryStationRequestDto, ErpDeliveryStationResponseDto, Guid, List, Task, IErpDeliveryStationAppService (+25 more)

### Community 141 - "TuTa.Wms.Application.Contracts"
Cohesion: 0.11
Nodes (16): CancellationToken, ILogger, Task, UnitOfWorkManager, ErpMoveSyncJob, DateTime, object, ErpMove (+8 more)

### Community 142 - "RecheckListService"
Cohesion: 0.09
Nodes (22): 2.0.0 (2021-02-18), 2.0.0-beta.1(2020-09-30), 2.0.0-beta.2 (2020-10-07), 2.0.0-rc.12 (2020-11-30), 2.0.0-rc.13 (2020-12-10), 2.3.0 (2021-04-10), 2.5.0(2021-06-20), 2.5.2(2021-06-27) (+14 more)

### Community 143 - "WmsModuleExtensionConfigurator"
Cohesion: 0.09
Nodes (20): author, email, name, url, bugs, url, engines, node (+12 more)

### Community 144 - "WmsSettingDefinitionProvider"
Cohesion: 0.14
Nodes (18): asnService, getAsnList(), pushReceipt(), { createConfirm }, detailTableColumns, getAsnListAsync(), getStatusName(), groupAsnListItems() (+10 more)

### Community 145 - "HandOutCell.vue"
Cohesion: 0.08
Nodes (8): GetPermissionListResultDto, IGetPermissionListResultDto, IPermissionGrantInfoDto, IPermissionGroupDto, IProviderInfoDto, PermissionGrantInfoDto, PermissionGroupDto, ProviderInfoDto

### Community 146 - "CellEventHandler"
Cohesion: 0.09
Nodes (22): ChangeEvent, DeepPartial, ElementAttributesProperty, ElementClass, ImportMetaEnv, Indexable, IntervalHandle, IntrinsicAttributes (+14 more)

### Community 147 - "BoxDisk.vue"
Cohesion: 0.13
Nodes (15): AbpApplicationDescriptor, TuTa.Wms.OpenIddict, IAbpApplicationManager, IOpenIddictScopeManager, IOpenIddictScopeRepository, IPermissionDataSeeder, IStringLocalizer, DataSeedContext (+7 more)

### Community 148 - "IncellByPeople.vue"
Cohesion: 0.06
Nodes (20): TuTa.Wms.AgvTasks, Wms.HttpApiTool, TuTa.Wms.AgvTasks.Dto, TuTa.Wms.Controllers.AgvTasks, TuTa.Wms.AgvTasks.Dtos, TuTa.Wms.Repositories.AgvTasks, DateTime, Guid (+12 more)

### Community 149 - "WmsDomainModule.cs"
Cohesion: 0.08
Nodes (22): IHttpClientFactory, IHttpContextAccessor, IValidatableObject, IEnumerable, ValidationContext, ValidationResult, LoginInput, Guid (+14 more)

### Community 150 - "IBarcodeListRepository"
Cohesion: 0.10
Nodes (13): Guid, LockUserInput, Guid, UpdateUserInput, IdentityUserManager, IIdentityUserAppService, IIdentityUserRepository, ListResultDto (+5 more)

### Community 151 - "ErpInboundOrderRequestDto"
Cohesion: 0.15
Nodes (9): PagedWarehouseQueryDto, WarehouseAreaUpdateDto, WarehouseUpdateDto, Guid, ILogger, List, PagedResultDto, Task (+1 more)

### Community 153 - "CheckInfoOfChkRsltList"
Cohesion: 0.10
Nodes (19): createOutStockTask(), boxCode, cell, cellCode, cellInputRef, createTask(), fetchLaneCellStatus(), goodsCode (+11 more)

### Community 154 - "IMaterialRepository"
Cohesion: 0.05
Nodes (29): SelectItem, barcodeListService, _BarcodeListServiceProxy, { createMessage }, modifyLoading, [openFullLoading, closeFullLoading], [registerEditForm, { getFieldsValue, setFieldsValue, validate }], [registerEditModal, { openModal: openEditModal, closeModal: closeEditModal }] (+21 more)

### Community 155 - "AbpModule"
Cohesion: 0.09
Nodes (15): AbpModule, ServiceConfigurationContext, ToolsModule, ServiceConfigurationContext, WmsApplicationModule, ServiceConfigurationContext, WmsDbMigratorModule, ServiceConfigurationContext (+7 more)

### Community 156 - "TuTa.Wms.sln"
Cohesion: 0.17
Nodes (8): ITransientDependency, Task, IWmsDbSchemaMigrator, Task, NullWmsDbSchemaMigrator, IServiceProvider, Task, EntityFrameworkCoreWmsDbSchemaMigrator

### Community 157 - "WmsPermissionDefinitionProvider"
Cohesion: 0.17
Nodes (13): HashSet, StockConsolidationMoveResult, StockConsolidationProgress, StockConsolidationOptions, Action, CancellationToken, DateTime, IConfiguration (+5 more)

### Community 158 - "ErpOutboundOrderRequestDto"
Cohesion: 0.16
Nodes (11): ICurrentTenant, IDataSeeder, ITenantRepository, IEnumerable, ILogger, Task, Tenant, WmsDbMigrationService (+3 more)

### Community 159 - "StorageBox.vue"
Cohesion: 0.10
Nodes (20): Microsoft.IdentityModel.Protocols.OpenIdConnect (7.0.3), Volo.Abp.AspNetCore.Authentication.OpenIdConnect (8.1.3), Volo.Abp.AspNetCore.Mvc.Client (8.1.3), Volo.Abp.Http.Client.IdentityModel.Web (8.1.3), Volo.Abp.Http.Client.Web (8.1.3), Volo.Abp.Identity.Web (8.1.3), Volo.Abp.SettingManagement.Web (8.1.3), Volo.Abp.TenantManagement.Web (8.1.3) (+12 more)

### Community 160 - "ErpOutboundItem"
Cohesion: 0.16
Nodes (4): Action, KeyValuePair, Task, IRedisClient

### Community 161 - "ChkResultList"
Cohesion: 0.18
Nodes (8): WarehouseAddDto, Guid, WarehouseAreaDto, Guid, List, PagedResultDto, Task, IWarehouseService

### Community 162 - "TuTa.Wms.Events"
Cohesion: 0.26
Nodes (7): Guid, HttpGet, HttpPost, List, PagedResultDto, Task, WarehouseController

### Community 163 - "WmsDomainService"
Cohesion: 0.15
Nodes (16): adjustmentTypeSelectItem, { createConfirm }, getStockAdjustmentsAllAsync(), getStockAdjustmentsAsync(), getTableData(), handleRestore(), handleStockRestoreEvent(), [openFullLoading, closeFullLoading] (+8 more)

### Community 164 - "HttpPost"
Cohesion: 0.10
Nodes (17): IDisposable, IHostApplicationLifetime, IHostedService, EmptyScope, CancellationToken, ILogger, Task, UnitOfWorkManager (+9 more)

### Community 165 - ".CreateCTUSkipStockInAsync"
Cohesion: 0.11
Nodes (16): autoAllocateStockDetailWithTotal(), columns3, DataItem, dataSource, editableData, emit, getdata(), good (+8 more)

### Community 166 - ".PreConfigureServices"
Cohesion: 0.12
Nodes (16): IChkResultListRepository, ILogger, object, Task, UnitOfWorkManager, StockEventHandler, ILogger, IStockInHistoryRepository (+8 more)

### Community 167 - "MaterialStatusChange.vue"
Cohesion: 0.20
Nodes (11): ICollection, IDictionary, IReadOnlyDictionary, Guid, List, StockConsolidationContainerSnapshot, StockConsolidationGroupPlan, StockConsolidationMovePlan (+3 more)

### Community 168 - ".CreateContainer"
Cohesion: 0.13
Nodes (14): CancellationToken, ILogger, Task, UnitOfWorkManager, ErpDepartmentSyncJob, ErpDepartment, CancellationToken, List (+6 more)

### Community 169 - ".TrackIf"
Cohesion: 0.12
Nodes (15): formData, formRef, { getFormRules }, getShow, loading, { notification, createErrorModal }, { prefixCls }, rememberMe (+7 more)

### Community 170 - "CellInfo"
Cohesion: 0.21
Nodes (6): ApplicationInitializationContext, IConfiguration, IServiceCollection, IWebHostEnvironment, ServiceConfigurationContext, WmsWebModule

### Community 171 - "WarehouseInfo"
Cohesion: 0.19
Nodes (14): qrcode, getErrorCorrectionLevel(), getOriginWidth(), renderQrCode(), canvasRoundRect(), drawLogo(), toCanvas(), ContentType (+6 more)

### Community 172 - "WmsController.cs"
Cohesion: 0.10
Nodes (18): boxbindcell(), boxCode, cellCode, goodheight, GoodsDetail, Ref1, Ref2, [registerGoodsDetailModal, { openModal: openGoodsDetailModal }] (+10 more)

### Community 173 - "TestModel.cs"
Cohesion: 0.13
Nodes (14): formData, formRef, { getFormRules }, getShow, handleLogin(), loading, { notification, createErrorModal }, { prefixCls } (+6 more)

### Community 174 - "CreateOutStockTask.vue"
Cohesion: 0.18
Nodes (7): TuTa.Wms.RecheckLists, TuTa.Wms.RecheckLists.Aggregates, TuTa.Wms.RecheckLists.ValueObjects, TuTa.Wms.RecheckLists.Entities, TuTa.Wms.Repositories.RecheckLists, RecheckItemStatus, RecheckListStatus

### Community 175 - "AcceptanceCalltest.vue"
Cohesion: 0.11
Nodes (13): AbpPageModel, TuTa.Wms.Web.Pages, TuTa.Wms.Pages, ILanguageProvider, IOpenIddictApplicationRepository, IReadOnlyList, List, OpenIddictApplication (+5 more)

### Community 176 - ".MoveStockAsync"
Cohesion: 0.27
Nodes (7): ApplicationService, ILogger, Task, UnitOfWork, ErpAsnStockInService, IErpAsnStockInService, WmsAppService

### Community 177 - "BoxIncell.vue"
Cohesion: 0.18
Nodes (10): CancellationTokenSource, IServiceScopeFactory, DateTime, StockConsolidationStatusDto, CancellationToken, IConfiguration, ILogger, object (+2 more)

### Community 178 - "CountInfoOfStock"
Cohesion: 0.18
Nodes (11): DateTime, object, ErpStockAftChk, CancellationToken, List, Task, IErpStockAftChkRepository, CancellationToken (+3 more)

### Community 179 - "WmsHttpApiModule"
Cohesion: 0.11
Nodes (16): DefaultBrandingProvider, IBrandingProvider, IndexModel, ITheme, Microsoft.AspNetCore.Http.Extensions, TuTa.Wms.Pages, Volo.Abp.AspNetCore.Mvc.UI.Theming, Volo.Abp.Ui.Branding (+8 more)

### Community 180 - "StockController"
Cohesion: 0.20
Nodes (10): DateTime, Guid, List, ErpInboundOrder, DateTime, Guid, List, Task (+2 more)

### Community 181 - "linliaoNodeModal.vue"
Cohesion: 0.13
Nodes (14): EntityDto, Guid, ILocalEventBus, ILogger, List, PagedResultDto, Task, CellService (+6 more)

### Community 182 - "MaterialBasic.vue"
Cohesion: 0.11
Nodes (18): Git 贡献提交规范, License, 交流, 使用 Gitpod, 准备, 后台整合示例, 如何贡献, 安装使用 (+10 more)

### Community 183 - "CancellationToken"
Cohesion: 0.12
Nodes (16): columns, count, dapdata, data(), dataSource, findtype, fliter, handleTableChange() (+8 more)

### Community 185 - "Guid"
Cohesion: 0.27
Nodes (6): Guid, ILogger, List, PagedResultDto, Task, ErpWorkshopMaterialTransferAppService

### Community 186 - "IEnumerable"
Cohesion: 0.19
Nodes (9): ErpNoPlanPickType, CancellationToken, List, Task, IErpNoPlanPickTypeRepository, CancellationToken, List, Task (+1 more)

### Community 187 - "ILogger"
Cohesion: 0.26
Nodes (7): IIdentityRoleAppService, IIdentityRoleRepository, Authorize, ListResultDto, PagedResultDto, Task, RoleAppService

### Community 188 - "List"
Cohesion: 0.11
Nodes (17): Microsoft.AspNetCore.Authentication.JwtBearer (8.0.0), Microsoft.Extensions.Hosting.Systemd (8.0.0), Volo.Abp.AspNetCore.Mvc.UI.MultiTenancy (8.1.3), net8.0, DistributedLock.Redis (1.0.2), Microsoft.AspNetCore.DataProtection.StackExchangeRedis (8.0.0), Microsoft.EntityFrameworkCore.Design (8.0.6), Serilog.AspNetCore (8.0.0) (+9 more)

### Community 189 - "AssemblyInfo.cs"
Cohesion: 0.20
Nodes (12): Guid, ErpMaterialDto, Guid, List, Task, IErpMaterialAppService, Guid, HttpDelete (+4 more)

### Community 190 - "ResponseDto"
Cohesion: 0.22
Nodes (9): Guid, ILogger, IRepository, Task, UnitOfWork, ErpOutboundOrderAppService, DateTime, Task (+1 more)

### Community 191 - "ServiceConfigurationContext"
Cohesion: 0.19
Nodes (10): DateTime, Guid, List, ErpOutboundOrder, OutboundOrderStatus, DateTime, Guid, List (+2 more)

### Community 192 - "AssemblyInfo.cs"
Cohesion: 0.18
Nodes (11): DateTime, object, ErpPickOrder, CancellationToken, List, Task, IErpPickOrderRepository, CancellationToken (+3 more)

### Community 193 - "AssemblyInfo.cs"
Cohesion: 0.12
Nodes (16): ExpandTransition, ExpandXTransition, FadeTransition, ScaleRotateTransition, ScaleTransition, ScrollXReverseTransition, ScrollXTransition, ScrollYReverseTransition (+8 more)

### Community 195 - "StockConsolidationMovePlan"
Cohesion: 0.11
Nodes (6): IUpdatePermissionDto, IUpdatePermissionsDto, IUpdateRolePermissionsInput, UpdatePermissionDto, UpdatePermissionsDto, UpdateRolePermissionsInput

### Community 196 - "StockConsolidationMoveResult"
Cohesion: 0.17
Nodes (5): bool, ILogger, IOptions, string, PlcHelper

### Community 197 - "StockConsolidationOptions"
Cohesion: 0.20
Nodes (11): DateTime, object, ErpRecheckNotifier, CancellationToken, List, Task, IErpRecheckNotifierRepository, CancellationToken (+3 more)

### Community 198 - "StockConsolidationPalletSnapshot"
Cohesion: 0.15
Nodes (14): formData, formRef, { getFormRules }, getShow, { handleBackLogin, getLoginState }, loading, { t }, { validForm } (+6 more)

### Community 199 - "StockConsolidationPlanner"
Cohesion: 0.28
Nodes (5): ApplicationInitializationContext, IConfiguration, IWebHostEnvironment, ServiceConfigurationContext, WmsHttpApiHostModule

### Community 200 - "StockConsolidationProgress"
Cohesion: 0.29
Nodes (5): componentMap, CellComponent(), ComponentProps, { t }, ComponentType

### Community 201 - "StockConsolidationSnapshot"
Cohesion: 0.18
Nodes (11): WarehouseType, CancellationToken, Guid, List, Task, IWarehouseRepository, DataSeedContext, Task (+3 more)

### Community 203 - "StockConsolidationStatusDto"
Cohesion: 0.15
Nodes (7): columns, executing, items, loading, orderCode, orderData, outboundOrderService

### Community 204 - "Task"
Cohesion: 0.13
Nodes (18): DateTime, Guid, List, ErpInboundItemDto, ErpInboundOrderDto, DateTime, Guid, List (+10 more)

### Community 205 - "IChkResultListRepository"
Cohesion: 0.23
Nodes (8): Guid, ILogger, Task, UnitOfWork, ErpInboundOrderAppService, DateTime, Task, ErpInboundOrderManager

### Community 206 - "SelectOrderDetail.vue"
Cohesion: 0.20
Nodes (8): TuTa.Wms.Erp.Events, ILocalEventHandler, DateTime, StockInCompletedEvent, IErpAsnStockInService, ILogger, Task, StockInCompletedEventHandler

### Community 207 - "MaterialPartialOut.vue"
Cohesion: 0.12
Nodes (6): EntityExtensionDto, ExtensionPropertyDto, IEntityExtensionDto, IExtensionPropertyDto, IModuleExtensionDto, ModuleExtensionDto

### Community 208 - "Stock.vue"
Cohesion: 0.14
Nodes (15): AccountController, IdentityDynamicClaimsPrincipalContributorCache, IdentityOptions, IdentitySecurityLogManager, ISettingProvider, Guid, HttpPost, IdentityUser (+7 more)

### Community 209 - "CreateOrder.vue"
Cohesion: 0.12
Nodes (6): FeatureDto, FeatureGroupDto, GetFeatureListResultDto, IFeatureDto, IFeatureGroupDto, IGetFeatureListResultDto

### Community 210 - "Good.vue"
Cohesion: 0.18
Nodes (16): getToken(), applyStatusResponse(), getStockConsolidationRequestOptions(), getStockConsolidationStatus(), handleStop(), readStatusField(), refreshing, refreshStatus() (+8 more)

### Community 211 - "AcceptanceCall.vue"
Cohesion: 0.13
Nodes (9): canGoBack, canGoForward, currentUrl, iframeUrl, isFullscreen, mobileFrame, navigateToUrl(), router (+1 more)

### Community 212 - "People.vue"
Cohesion: 0.20
Nodes (10): dependencies, fs-extra, koa, koa2-cors, koa-body, koa-bodyparser, koa-route, koa-router (+2 more)

### Community 213 - "Peoples.vue"
Cohesion: 0.22
Nodes (9): BackgroundService, TimeSpan, CancellationToken, DateTime, IConfiguration, ILogger, string, Task (+1 more)

### Community 214 - ".ModifyChkResultList"
Cohesion: 0.23
Nodes (8): TuTa.Wms.Controllers.StockConsolidations, Task, IStockConsolidationService, HttpGet, HttpPost, SwaggerOperation, Task, StockConsolidationController

### Community 215 - "CreateStockTask.vue"
Cohesion: 0.11
Nodes (10): TuTa.Wms.StockInHistories.ValueObjects, DateTime, IEnumerable, CheckInfoOfStockInHistory, IEnumerable, MaterialInfoOfStockInHistory, IEnumerable, StockPlaceOfStockInHistory (+2 more)

### Community 216 - "MaterialOut.vue"
Cohesion: 0.13
Nodes (13): Microsoft.Extensions.Http.Polly (8.0.0), Volo.Abp.Account.HttpApi.Client (8.1.3), Volo.Abp.Http.Client.IdentityModel (8.1.3), Volo.Abp.Identity.HttpApi.Client (8.1.3), Volo.Abp.PermissionManagement.HttpApi.Client (8.1.3), net8.0, netstandard2.0, netstandard2.1 (+5 more)

### Community 217 - "Auto.vue"
Cohesion: 0.24
Nodes (8): DateTime, List, InboundOrderStatus, DateTime, Guid, List, Task, IErpInboundOrderRepository

### Community 218 - "LockPage.vue"
Cohesion: 0.29
Nodes (7): DateTime, List, DateTime, Guid, List, Task, IErpOutboundOrderRepository

### Community 219 - "LoginForm.vue"
Cohesion: 0.13
Nodes (14): net8.0, DistributedLock.Redis (1.0.2), Microsoft.AspNetCore.DataProtection.StackExchangeRedis (8.0.0), Serilog.AspNetCore (8.0.0), Serilog.Sinks.Async (1.5.0), Volo.Abp.Account.Application (8.1.3), Volo.Abp.Account.HttpApi (8.1.3), Volo.Abp.Account.Web.OpenIddict (8.1.3) (+6 more)

### Community 220 - "Cell.vue"
Cohesion: 0.16
Nodes (11): BoxManager, IBoxRepository, ILogger, object, Task, UnitOfWorkManager, StockEventHandler, Guid (+3 more)

### Community 221 - "MobileBrowser.vue"
Cohesion: 0.26
Nodes (8): DateTime, Guid, HttpDelete, HttpGet, HttpPut, List, Task, ErpOutboundOrderController

### Community 222 - "LoginForm.vue"
Cohesion: 0.13
Nodes (15): [2.0.1](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0...v2.0.1) (2021-02-21), [2.0.2](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0...v2.0.2) (2021-03-03), [2.0.3](https://github.com/anncwb/vue-vben-admin/compare/v2.0.2...v2.0.3) (2021-03-07), [2.1.0](https://github.com/anncwb/vue-vben-admin/compare/v2.0.3...v2.1.0) (2021-03-15), Bug Fixes, Bug Fixes, Bug Fixes, Bug Fixes (+7 more)

### Community 224 - "StockConsolidation.vue"
Cohesion: 0.17
Nodes (11): DateTime, EnumCheckResult, EnumCheckType, RecheckResultGettedEvent, ILogger, IStockRepository, object, StocksManager (+3 more)

### Community 225 - "ErpOutboundOrderController"
Cohesion: 0.17
Nodes (3): RecheckListServiceProxy, _PickNotifierServiceProxy, _RecheckListServiceProxy

### Community 226 - "LaneCellChips.vue"
Cohesion: 0.19
Nodes (9): Wms.PlcTool, EnumPlcTagType, EnumQuality, EnumTagAccess, bool, string, PlcTag, string (+1 more)

### Community 227 - "AcceptanceCall2.vue"
Cohesion: 0.13
Nodes (10): TuTa.Wms.HttpApi.Client.ConsoleTestApp, IProfileAppService, Task, ClientDemoService, CancellationToken, IConfiguration, Task, ConsoleTestAppHostedService (+2 more)

### Community 228 - "task.vue"
Cohesion: 0.14
Nodes (13): NSubstitute (5.1.0), NSubstitute.Analyzers.CSharp (1.0.16), Shouldly (4.2.1), Volo.Abp.Authorization (8.1.3), Volo.Abp.BackgroundJobs.Abstractions (8.1.3), Volo.Abp.TestBase (8.1.3), xunit (2.6.1), xunit.extensibility.execution (2.6.1) (+5 more)

### Community 229 - "Outstock.vue"
Cohesion: 0.18
Nodes (11): Guid, DepartmentDto, List, Task, IDepartmentService, HttpGet, HttpPost, List (+3 more)

### Community 230 - "TuTa.Wms.Warehouses.Dtos"
Cohesion: 0.09
Nodes (15): EventId, Exception, ILoggerProvider, LogLevel, Func, ILogger, string, SqliteLogger (+7 more)

### Community 231 - "OutStockOrder.vue"
Cohesion: 0.38
Nodes (5): CancellationToken, Guid, List, Task, EfCoreWarehouseRepository

### Community 232 - "AgvTaskManagement.vue"
Cohesion: 0.14
Nodes (13): [2.0.0-beta.1](https://github.com/anncwb/vue-vben-admin/compare/2f268ca8f43d98687ffd809e2c1d140d29033bd6...2.0.0-beta.1) (2020-09-30), [2.0.0-beta.2](https://github.com/anncwb/vue-vben-admin/compare/2.0.0-beta.1...2.0.0-beta.2) (2020-10-07), [2.0.0-beta.3](https://github.com/anncwb/vue-vben-admin/compare/2.0.0-beta.2...2.0.0-beta.3) (2020-10-07), [2.0.0](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.18...v2.0.0) (2021-02-17), [2.3.0](https://github.com/anncwb/vue-vben-admin/compare/v2.2.0...v2.3.0) (2021-04-10), Bug Fixes, Bug Fixes, Bug Fixes (+5 more)

### Community 233 - "TuTa.Wms.RecheckLists.Dtos"
Cohesion: 0.14
Nodes (14): [2.5.1](https://github.com/anncwb/vue-vben-admin/compare/v2.4.0...v2.5.1) (2021-06-26), [2.5.1](https://github.com/anncwb/vue-vben-admin/compare/v2.4.0...v2.5.1) (2021-06-26), [2.6.0](https://github.com/anncwb/vue-vben-admin/compare/v2.5.2...v2.6.0) (2021-07-04), Bug Fixes, Bug Fixes, Bug Fixes, Features, Features (+6 more)

### Community 234 - "TestScanAsn.vue"
Cohesion: 0.26
Nodes (8): addClass(), getBoundingClientRect(), getViewportOffset(), hackCss(), hasClass(), removeClass(), trim(), ViewportOffsetResult

### Community 237 - "BarcodeList.vue"
Cohesion: 0.19
Nodes (11): cellStatusLabelMap, getRunStatusLabel(), getRunStatusShort(), getStatusLabel(), normalizeRunStatus(), props, runStatusChipClasses(), runStatusChipTitle() (+3 more)

### Community 238 - "StockAdjustment.vue"
Cohesion: 0.16
Nodes (12): columns2, count, data(), dataSource, findtype, fliter, getPagedCheckInItems(), good (+4 more)

### Community 239 - "index.vue"
Cohesion: 0.15
Nodes (9): mockAsn26062444928Data, asnCode, asnDataList, asnInput, asnService, loading, MockAsnItem, saving (+1 more)

### Community 240 - "ContainerUnbind.vue"
Cohesion: 0.18
Nodes (11): devDependencies, nodemon, pm2, rimraf, ts-node, tsup, @types/koa, @types/koa-bodyparser (+3 more)

### Community 241 - "OutStockSort.vue"
Cohesion: 0.19
Nodes (7): ApplicationShutdownContext, WmsEntityFrameworkCoreTestBase, IServiceCollection, ServiceConfigurationContext, SqliteConnection, WmsEntityFrameworkCoreTestModule, WmsTestBase

### Community 242 - "index.vue"
Cohesion: 0.15
Nodes (10): Volo.Abp.EntityFrameworkCore.Sqlite (8.1.3), net8.0, Microsoft.NET.Test.Sdk (17.8.0), Microsoft.NET.Sdk, net8.0, Microsoft.NET.Test.Sdk (17.8.0), Microsoft.NET.Sdk, net8.0 (+2 more)

### Community 243 - "MobileForm.vue"
Cohesion: 0.20
Nodes (8): formData, formRef, { getFormRules }, getShow, { handleBackLogin, getLoginState }, loading, { t }, { validForm }

### Community 244 - "RegisterForm.vue"
Cohesion: 0.17
Nodes (13): DateTime, Guid, List, CreateFromBarcodeDto, ErpOutboundItemDto, ErpOutboundOrderDto, ErpOutboundRecordDto, DateTime (+5 more)

### Community 246 - "EfCoreBarcodeListRepository"
Cohesion: 0.17
Nodes (8): TuTa.Wms.Users.Dtos, TuTa.Wms.Permissions, TuTa.Wms.Users, string, WmsPermissions, AbpLoginResult, LoginResultType, UserLoginInfo

### Community 247 - "ForgetPasswordForm.vue"
Cohesion: 0.13
Nodes (15): emit, getClass, { getIsMobile }, getIsNotData, handleClose(), { handleSearch, searchResult, keyword, activeIndex, handleEnter, handleMouseenter }, inputRef, { prefixCls } (+7 more)

### Community 248 - "AbpRole.vue"
Cohesion: 0.27
Nodes (10): setup(), BAR_MAP, extend(), renderThumbStyle(), toObject(), repeatDirective, off(), on() (+2 more)

### Community 249 - "LoginFormTitle.vue"
Cohesion: 0.15
Nodes (12): compilerOptions, baseUrl, declaration, emitDecoratorMetadata, esModuleInterop, experimentalDecorators, module, outDir (+4 more)

### Community 250 - "ErpInboundItem"
Cohesion: 0.17
Nodes (8): AbpViewComponent, TuTa.Wms.Web.Components.Toolbar.LoginLink, IToolbarConfigurationContext, IToolbarContributor, IViewComponentResult, LoginLinkViewComponent, Task, WmsToolbarContributor

### Community 252 - "IncellHis copy 2.vue"
Cohesion: 0.17
Nodes (11): Abp (9.2.2), NPOI (2.7.2), Volo.Abp.Identity.Application (8.1.3), Volo.Abp.PermissionManagement.Application (8.1.3), Volo.Abp.TenantManagement.Application (8.1.3), net8.0, Microsoft.EntityFrameworkCore.Design (8.0.6), Volo.Abp.Account.Application (8.1.3) (+3 more)

### Community 253 - "OutcellHis.vue"
Cohesion: 0.17
Nodes (11): Microsoft.EntityFrameworkCore.Tools (8.0.0), Volo.Abp.AuditLogging.EntityFrameworkCore (8.1.3), Volo.Abp.BackgroundJobs.EntityFrameworkCore (8.1.3), Volo.Abp.EntityFrameworkCore.MySQL (8.1.3), Volo.Abp.EntityFrameworkCore.SqlServer (8.1.3), Volo.Abp.Identity.EntityFrameworkCore (8.1.3), Volo.Abp.OpenIddict.EntityFrameworkCore (8.1.3), Volo.Abp.PermissionManagement.EntityFrameworkCore (8.1.3) (+3 more)

### Community 254 - "AbpUser.vue"
Cohesion: 0.17
Nodes (11): Microsoft.Extensions.FileProviders.Embedded (8.0.0), Volo.Abp.AuditLogging.Domain.Shared (8.1.3), Volo.Abp.BackgroundJobs.Domain.Shared (8.1.3), Volo.Abp.Identity.Domain.Shared (8.1.3), Volo.Abp.OpenIddict.Domain.Shared (8.1.3), Volo.Abp.PermissionManagement.Domain.Shared (8.1.3), Volo.Abp.TenantManagement.Domain.Shared (8.1.3), net8.0 (+3 more)

### Community 255 - "AsnList.vue"
Cohesion: 0.24
Nodes (8): PagedAndSortedResultRequestDto, DateTime, ErpWorkshopMaterialTransferQueryDto, DateTime, ErpWorkstationMaterialReceiptQueryDto, ErpWorkstationMaterialReceiptRequestDto, ErpWorkstationMaterialReceiptResponseDto, HttpPost

### Community 256 - "Login.vue"
Cohesion: 0.22
Nodes (7): Guid, Material, CancellationToken, Guid, List, Task, EfCoreMaterialRepository

### Community 258 - "IncellHis.vue"
Cohesion: 0.20
Nodes (9): Guid, PickListStockOutEvent, ILogger, IStockRepository, LocalEventBus, object, Task, UnitOfWorkManager (+1 more)

### Community 259 - "OutboundOrder.vue"
Cohesion: 0.17
Nodes (12): [2.4.1](https://github.com/anncwb/vue-vben-admin/compare/v2.4.0...v2.4.1) (2021-06-01), [2.4.2](https://github.com/anncwb/vue-vben-admin/compare/v2.4.0...v2.4.2) (2021-06-09), [2.5.0](https://github.com/anncwb/vue-vben-admin/compare/v2.4.0...v2.5.0) (2021-06-20), Bug Fixes, Bug Fixes, Bug Fixes, Features, Features (+4 more)

### Community 260 - "WorkshopReceiptStatistics.vue"
Cohesion: 0.24
Nodes (9): convertFlowElementToEdge(), convertFlowElementToNode(), toLogicFlowData(), TurboType, BpmnNode, configDefaultDndPanel(), nodeList, setup() (+1 more)

### Community 261 - "createStockTask"
Cohesion: 0.18
Nodes (11): FreezeStockEvent, Guid, ReCheckStockOutEvent, UnFreezeStockEvent, ILogger, IStockRepository, LocalEventBus, object (+3 more)

### Community 262 - "Exception.vue"
Cohesion: 0.17
Nodes (4): IdentityRoleUpdateDto, IIdentityRoleUpdateDto, IUpdateRoleInput, UpdateRoleInput

### Community 263 - "index.vue"
Cohesion: 0.17
Nodes (4): IdentityUserUpdateDto, IIdentityUserUpdateDto, IUpdateUserInput, UpdateUserInput

### Community 264 - "MaterialStockStatistics.vue"
Cohesion: 0.17
Nodes (4): IUpdateFeatureDto, IUpdateFeaturesDto, UpdateFeatureDto, UpdateFeaturesDto

### Community 266 - "App.vue"
Cohesion: 0.18
Nodes (8): Microsoft.Data.Sqlite.Core (8.0.0), Microsoft.EntityFrameworkCore.Sqlite (8.0.0), Newtonsoft.Json (13.0.3), RestSharp (110.2.0), StackExchange.Redis (2.7.33), Volo.Abp.Core (8.0.0), net8.0, Microsoft.NET.Sdk

### Community 267 - "index.vue"
Cohesion: 0.18
Nodes (10): Swashbuckle.AspNetCore.Annotations (6.6.2), Volo.Abp.Identity.AspNetCore (8.1.3), Volo.Abp.Identity.HttpApi (8.1.3), Volo.Abp.PermissionManagement.HttpApi (8.1.3), net8.0, Volo.Abp.Account.HttpApi (8.1.3), Volo.Abp.Account.Web.OpenIddict (8.1.3), Volo.Abp.AspNetCore.Mvc (8.1.3) (+2 more)

### Community 268 - "OidcSignIn.vue"
Cohesion: 0.36
Nodes (6): IActionResult, HttpGet, HttpPost, SwaggerOperation, Task, ErpAsnController

### Community 269 - "OidcSignOut.vue"
Cohesion: 0.33
Nodes (4): IPermissionDefinitionContext, LocalizableString, PermissionDefinitionProvider, WmsPermissionDefinitionProvider

### Community 270 - "EditCell.vue"
Cohesion: 0.15
Nodes (11): StockConsolidationCellParser, Dictionary, StockConsolidationAgvQueryResult, StockConsolidationAgvTaskSnapshot, StockConsolidationCellPosition, StockConsolidationCellSnapshot, StockConsolidationSnapshot, StockConsolidationSnapshotResult (+3 more)

### Community 271 - "ExcelOut.vue"
Cohesion: 0.31
Nodes (7): ToolbarTypeEnum, setup(), NodeItem, ToolbarConfig, Instance, key, useFlowChartContext()

### Community 273 - "CurrentCultureDto"
Cohesion: 0.18
Nodes (4): CurrentCultureDto, DateTimeFormatDto, ICurrentCultureDto, IDateTimeFormatDto

### Community 274 - "ExtensionEnumDto"
Cohesion: 0.18
Nodes (4): ExtensionEnumDto, ExtensionEnumFieldDto, IExtensionEnumDto, IExtensionEnumFieldDto

### Community 275 - "IStringValueType"
Cohesion: 0.18
Nodes (4): IIStringValueType, IIValueValidator, IStringValueType, IValueValidator

### Community 276 - "ListResultDto_1OfOfUserDataAndAbstractionsAnd_0AndCulture_neutralAndPublicKeyToken_null"
Cohesion: 0.15
Nodes (6): IEnumerable, CountInfoOfChkRsltList, IEnumerable, MaterialInfoOfChkRsltList, IEnumerable, SupplierInfoOfChkRsltList

### Community 277 - "PropertyApiDescriptionModel"
Cohesion: 0.18
Nodes (4): IPropertyApiDescriptionModel, ITypeApiDescriptionModel, PropertyApiDescriptionModel, TypeApiDescriptionModel

### Community 278 - "TimeZone"
Cohesion: 0.08
Nodes (8): IanaTimeZone, IIanaTimeZone, ITimeZone, ITimingDto, IWindowsTimeZone, TimeZone, TimingDto, WindowsTimeZone

### Community 279 - "ContainerUnbind.vue"
Cohesion: 0.38
Nodes (3): List, PuArrVouchAddRequestDto, PuArrVouchDetailRequestDto

### Community 280 - "index.vue"
Cohesion: 0.06
Nodes (23): TuTa.Wms.Repositories.Cells, TuTa.Wms.EntityFrameworkCore, TuTa.Wms.Repositories.StockInHistories, TuTa.Wms.Repositories.Erp, TuTa.Wms.Repositories.PickLists, TuTa.Wms.Repositories.Boxes, TuTa.Wms.Repositories.InBoundLists, Wms.Repositories.Warehouses (+15 more)

### Community 281 - "TuTa.Wms.Domain.csproj"
Cohesion: 0.20
Nodes (9): Microsoft.Extensions.Http (8.0.0), Volo.Abp.AuditLogging.Domain (8.1.3), Volo.Abp.Identity.Domain (8.1.3), Volo.Abp.OpenIddict.Domain (8.1.3), Volo.Abp.PermissionManagement.Domain.Identity (8.1.3), Volo.Abp.PermissionManagement.Domain.OpenIddict (8.1.3), Volo.Abp.TenantManagement.Domain (8.1.3), net8.0 (+1 more)

### Community 282 - "TuTa.Wms.DbMigrator.csproj"
Cohesion: 0.20
Nodes (9): Serilog.Extensions.Logging (8.0.0), Serilog.Sinks.Console (5.0.1), net8.0, Microsoft.EntityFrameworkCore.Design (8.0.6), Microsoft.Extensions.Hosting (8.0.0), Serilog.Sinks.Async (1.5.0), Serilog.Sinks.File (5.0.0), Volo.Abp.Autofac (8.1.3) (+1 more)

### Community 283 - "TuTa.Wms.Application.Contracts.csproj"
Cohesion: 0.20
Nodes (9): Volo.Abp.Account.Application.Contracts (8.1.3), Volo.Abp.Identity.Application.Contracts (8.1.3), Volo.Abp.ObjectExtending (8.1.3), Volo.Abp.PermissionManagement.Application.Contracts (8.1.3), Volo.Abp.TenantManagement.Application.Contracts (8.1.3), net8.0, netstandard2.0, netstandard2.1 (+1 more)

### Community 284 - "Scrollbar.vue"
Cohesion: 0.40
Nodes (7): setup(), domSymbol, useWatermark(), useRafThrottle(), addResizeListener(), removeResizeListener(), resizeHandler()

### Community 288 - "index.d.ts"
Cohesion: 0.20
Nodes (9): ComponentElRef, ComponentRef, ElRef, EmitType, Fn, LabelValueOptions, PromiseFn, RefType (+1 more)

### Community 289 - "WmsSettingDefinitionProvider"
Cohesion: 0.22
Nodes (6): TuTa.Wms._Settings, ISettingDefinitionContext, SettingDefinitionProvider, WmsSettingDefinitionProvider, string, WmsSettings

### Community 290 - "HubMsgQHelper"
Cohesion: 0.25
Nodes (6): Wms.SignalRTool, ISingletonDependency, Queue, object, HubMessage, HubMsgQHelper

### Community 291 - "CellEventHandler"
Cohesion: 0.33
Nodes (7): Guid, Department, CancellationToken, Guid, List, Task, EfCoreDepartmentRepository

### Community 292 - "[2.7.0](https://github.com/anncwb/vue-vben-admin/compare/v2.5.9...v2.7.0) (2021-08-03)"
Cohesion: 0.22
Nodes (9): [2.6.1](https://github.com/anncwb/vue-vben-admin/compare/v2.6.0...v2.6.1) (2021-07-19), [2.7.0](https://github.com/anncwb/vue-vben-admin/compare/v2.5.9...v2.7.0) (2021-08-03), Bug Fixes, Bug Fixes, Features, Features, Performance Improvements, Performance Improvements (+1 more)

### Community 293 - "functional.ts"
Cohesion: 0.31
Nodes (6): createImgPreview(), ImageItem, ImageProps, Options, PreviewActions, Props

### Community 294 - "WmsMenuContributor"
Cohesion: 0.18
Nodes (9): CountButton, CountdownInput, formData, formRef, { getFormRules }, getShow, { handleBackLogin, getLoginState }, loading (+1 more)

### Community 295 - "Index.cshtml"
Cohesion: 0.25
Nodes (7): TuTa.Wms.Web.Pages.IndexModel, ICurrentUser, IHtmlLocalizer<WmsResource>, Microsoft.AspNetCore.Mvc.Localization, TuTa.Wms.Localization, Volo.Abp.Users, WmsResource

### Community 296 - "ErpInboundOrderRequestDto"
Cohesion: 0.39
Nodes (5): List, ErpInboundItemRequestDto, ErpInboundOrderRequestDto, ErpInboundOrderResponseDto, HttpPost

### Community 297 - "LLBJDAddRequestDto"
Cohesion: 0.46
Nodes (5): List, LLBJDAddRequestDto, LLBJDAddResponseDto, LLBJDDataItem, LLBJDDetail

### Community 298 - "IMaterialRepository"
Cohesion: 0.21
Nodes (9): TuTa.Wms.BarcodeLists.Dtos, TuTa.Wms.Controllers.BarcodeLists, DateTime, BarcodeDto, Task, IBarcodeListService, HttpGet, Task (+1 more)

### Community 299 - "dongfangdianzi"
Cohesion: 0.25
Nodes (7): dongfangdianzi, 介绍, 使用说明, 参与贡献, 安装教程, 特技, 软件架构

### Community 300 - "dongfangdianzi"
Cohesion: 0.25
Nodes (7): Contribution, Description, dongfangdianzi, Gitee Feature, Installation, Instructions, Software Architecture

### Community 301 - "ExpandTransition.ts"
Cohesion: 0.29
Nodes (3): afterLeave(), HTMLExpandElement, resetStyles()

### Community 302 - "index.ts"
Cohesion: 0.25
Nodes (6): EventType, options, RippleDirective, RippleOptions, RippleProto, setProps()

### Community 304 - "download.ts"
Cohesion: 0.57
Nodes (5): dataURLtoBlob(), urlToBase64(), downloadByBase64(), downloadByData(), downloadByOnlineUrl()

### Community 305 - "FakeCurrentPrincipalAccessor"
Cohesion: 0.38
Nodes (4): ClaimsPrincipal, TuTa.Wms.Security, ThreadCurrentPrincipalAccessor, FakeCurrentPrincipalAccessor

### Community 306 - "ErpOutboundOrderRequestDto"
Cohesion: 0.48
Nodes (4): List, ErpOutboundItemRequestDto, ErpOutboundOrderRequestDto, ErpOutboundOrderResponseDto

### Community 309 - "ErpInboundItem"
Cohesion: 0.40
Nodes (3): Guid, ErpInboundItem, InboundItemStatus

### Community 310 - "[2.2.0](https://github.com/anncwb/vue-vben-admin/compare/v2.1.1...v2.2.0) (2021-04-07)"
Cohesion: 0.29
Nodes (7): [2.1.1](https://github.com/anncwb/vue-vben-admin/compare/v2.1.0...v2.1.1) (2021-03-25), [2.2.0](https://github.com/anncwb/vue-vben-admin/compare/v2.1.1...v2.2.0) (2021-04-07), Bug Fixes, Bug Fixes, Features, Features, Performance Improvements

### Community 311 - "2.0.0-rc.15 (2020-12-31)"
Cohesion: 0.29
Nodes (7): 2.0.0-rc.15 (2020-12-31), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Table destructive update, ✨ Table reconstruction

### Community 312 - "2.0.0-rc.15 (2020-12-31)"
Cohesion: 0.29
Nodes (7): 2.0.0-rc.15 (2020-12-31), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ 表格破坏性更新, ✨ 表格重构

### Community 313 - "scripts"
Cohesion: 0.29
Nodes (7): setup(), scripts, build, prod, restart, start, stop

### Community 314 - "column.ts"
Cohesion: 0.29
Nodes (6): ColumnFilterItem, ColumnProps, CustomRenderFunction, FilterDropdownProps, RecordProps, SortOrder

### Community 315 - "AbpTenantServiceProxy"
Cohesion: 0.17
Nodes (4): ApplicationLocalizationDto, ApplicationLocalizationResourceDto, IApplicationLocalizationDto, IApplicationLocalizationResourceDto

### Community 316 - "BoxBindCellEvent.cs"
Cohesion: 0.33
Nodes (4): TuTa.Wms.Events, Guid, BoxBindCellEvent, StockOutHistoryDelEvent

### Community 317 - "WmsTestDataSeedContributor"
Cohesion: 0.33
Nodes (4): IDataSeedContributor, DataSeedContext, Task, WmsTestDataSeedContributor

### Community 319 - ".PushCGRKDAddAsync"
Cohesion: 0.52
Nodes (3): CGRKDAddRequestDto, CGRKDAddResponseDto, CGRKDParams

### Community 320 - "package.json"
Cohesion: 0.33
Nodes (5): dependencies, @abp/aspnetcore.mvc.ui.theme.leptonxlite, name, private, version

### Community 321 - "WmsAuthServerModule"
Cohesion: 0.40
Nodes (3): ApplicationInitializationContext, ServiceConfigurationContext, WmsAuthServerModule

### Community 322 - "Program"
Cohesion: 0.40
Nodes (3): IHostBuilder, Task, Program

### Community 323 - "package.json"
Cohesion: 0.33
Nodes (5): dependencies, @abp/aspnetcore.mvc.ui.theme.leptonxlite, name, private, version

### Community 324 - "Program"
Cohesion: 0.40
Nodes (3): IHostBuilder, Task, Program

### Community 325 - "2.0.0-rc.10 (2020-11-13)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.10 (2020-11-13), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, ⚡ Wip

### Community 326 - "2.0.0-rc.14 (2020-12-15)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.14 (2020-12-15), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 327 - "2.0.0-rc.6 (2020-10-28)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.6 (2020-10-28), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 328 - "2.0.0-rc.7 (2020-10-31)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.7 (2020-10-31), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 329 - "2.0.0-rc.9 (2020-11-9)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.9 (2020-11-9), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 330 - "2.1.1 (2021-03-26)"
Cohesion: 0.33
Nodes (6): 2.1.1 (2021-03-26), 🐛 Bug Fixes, 🎫 Chores, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 331 - "Breaking changes"
Cohesion: 0.33
Nodes (6): Breaking changes, 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 332 - "(Breaking changes) Breaking changes"
Cohesion: 0.33
Nodes (6): (Breaking changes) Breaking changes, 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 333 - "(破坏性更新) Breaking changes"
Cohesion: 0.33
Nodes (6): 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, (破坏性更新) Breaking changes

### Community 334 - "2.0.0-rc.10 (2020-11-13)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.10 (2020-11-13), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, ⚡ Wip

### Community 335 - "2.0.0-rc.14 (2020-12-15)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.14 (2020-12-15), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 336 - "2.0.0-rc.6 (2020-10-28)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.6 (2020-10-28), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 337 - "2.0.0-rc.7 (2020-10-31)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.7 (2020-10-31), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 338 - "2.0.0-rc.9 (2020-11-9)"
Cohesion: 0.33
Nodes (6): 2.0.0-rc.9 (2020-11-9), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 339 - "(破坏性更新) Breaking changes"
Cohesion: 0.33
Nodes (6): 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, (破坏性更新) Breaking changes

### Community 340 - "(破坏性更新) Breaking changes"
Cohesion: 0.33
Nodes (6): 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, (破坏性更新) Breaking changes

### Community 341 - "(破坏性更新) Breaking changes"
Cohesion: 0.33
Nodes (6): 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, (破坏性更新) Breaking changes

### Community 353 - "CGRKDAddRequestDto"
Cohesion: 0.18
Nodes (7): confirmLoading, data, emit, options, props, [registerModal, { changeOkLoading, closeModal }], textarea01

### Community 354 - "CGRKDParams"
Cohesion: 0.25
Nodes (4): ServiceConfigurationContext, WmsApplicationContractsModule, OneTimeRunner, WmsDtoExtensions

### Community 365 - "ErpInboundOrderResponseDto"
Cohesion: 0.38
Nodes (3): Guid, ErpOutboundItem, OutboundItemStatus

### Community 371 - "ErpOutboundOrderRequestDto"
Cohesion: 0.38
Nodes (3): ApplicationInitializationContext, ServiceConfigurationContext, WmsTestBaseModule

### Community 384 - "ErpWorkstationMaterialRequestResponseDto"
Cohesion: 0.02
Nodes (33): AgvTaskPagedQueryDto, DepartmentDto, ErpInboundItemRequestDto, ErpInboundOrderResponseDto, IAgvTaskPagedQueryDto, ICGRKDAddResponseDto, IdentityUserUpdateRolesDto, IDepartmentDto (+25 more)

### Community 386 - "GetBySkipDto"
Cohesion: 0.20
Nodes (5): columns, items, loading, sortCode, sortData

### Community 387 - "GetChkByBarcodeDto"
Cohesion: 0.31
Nodes (7): List, ConfigOptions, Jwt, MjjAvoidPos, PickTypeMap, PlcHeartBeatSet, Settings

### Community 389 - "IdentityUserUpdateRolesDto"
Cohesion: 0.44
Nodes (5): CancellationToken, Guid, List, Task, EfCoreBarcodeListRepository

### Community 394 - "MaterialDto"
Cohesion: 0.44
Nodes (5): CancellationToken, Guid, List, Task, EfCoreChkResultListRepository

### Community 395 - "MaterialImportResultDto"
Cohesion: 0.31
Nodes (7): getYdStockTableListAsync(), [registerTable], _StockServiceProxy, { t }, { t }, ydStockSearchFormSchema, ydStockTableColumns

### Community 396 - "MaterialUpdateDto"
Cohesion: 0.36
Nodes (5): DateTime, EnumCheckResult, EnumCheckType, IEnumerable, CheckInfoOfChkRsltList

### Community 403 - "PagedMaterialsQueryDto"
Cohesion: 0.33
Nodes (5): dependencies, @abp/aspnetcore.mvc.ui.theme.leptonxlite, name, private, version

### Community 419 - "PickItemDto"
Cohesion: 0.09
Nodes (10): TuTa.Wms.Stocks.Dtos, TuTa.Wms.Repositories.Moves, TuTa.Wms.ChkResultLists, TuTa.Wms.ChkResultLists.Dtos, TuTa.Wms.Moves.Aggregates, TuTa.Wms.Controllers.InBoundLists, TuTa.Wms.Moves, MaterialToInBoundDto (+2 more)

### Community 433 - "SkipInDto"
Cohesion: 0.50
Nodes (4): 2.1.0 (2021-03-15), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 436 - "StockCreateDto"
Cohesion: 0.50
Nodes (4): 2.0.3 (2021-03-07), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 438 - "StockDto"
Cohesion: 0.67
Nodes (3): (Breaking changes) Breaking changes, 🐛 Bug Fixes, ✨ Features

### Community 451 - "WmsDomainService"
Cohesion: 0.40
Nodes (5): IDistributedEventBus, IObjectMapper, Type, IUnitOfWorkManager, WmsDomainService

### Community 455 - "[2.0.0-rc.14](https://github.com/anncwb/vue-vben-admin/compare/2.0.0-beta.3...v2.0.0-rc.14) (2020-12-15)"
Cohesion: 0.40
Nodes (5): [2.0.0-rc.14](https://github.com/anncwb/vue-vben-admin/compare/2.0.0-beta.3...v2.0.0-rc.14) (2020-12-15), Bug Fixes, Features, Performance Improvements, Reverts

### Community 456 - "[2.0.0-rc.18](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.17...v2.0.0-rc.18) (2021-02-04)"
Cohesion: 0.40
Nodes (5): [2.0.0-rc.18](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.17...v2.0.0-rc.18) (2021-02-04), Bug Fixes, Features, Performance Improvements, Reverts

### Community 457 - "2.0.0-beta.5 (2020-10-10)"
Cohesion: 0.40
Nodes (5): 2.0.0-beta.5 (2020-10-10), 🐛 Bug Fixes, 🎫 Chores, ♻ Code Refactoring, ⚡ Performance Improvements

### Community 458 - "2.0.0-beta.7 (2020-10-12)"
Cohesion: 0.40
Nodes (5): 2.0.0-beta.7 (2020-10-12), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 459 - "2.0.0-rc.11 (2020-11-18)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.11 (2020-11-18), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 460 - "2.0.0-rc.1 (2020-10-14)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.1 (2020-10-14), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 461 - "2.0.0-rc.2 (2020-10-17)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.2 (2020-10-17), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 462 - "2.0.0-rc.3 (2020-10-19)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.3 (2020-10-19), 🐛 Bug Fixes, 🎫 Chores, Docs, ✨ Features

### Community 463 - "2.0.0-rc.4 (2020-10-21)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.4 (2020-10-21), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 464 - "# 2.0.0-rc.5 (2020-10-26)"
Cohesion: 0.40
Nodes (5): # 2.0.0-rc.5 (2020-10-26), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 465 - "2.0.0-rc.8 (2020-11-2)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.8 (2020-11-2), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 466 - "2.0.1 (2021-02-21)"
Cohesion: 0.40
Nodes (5): 2.0.1 (2021-02-21), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 467 - "2.0.2 (2021-03-04)"
Cohesion: 0.40
Nodes (5): 2.0.2 (2021-03-04), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 468 - "(Breaking changes) Breaking changes"
Cohesion: 0.40
Nodes (5): (Breaking changes) Breaking changes, 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 469 - "2.0.0-beta.5 (2020-10-10)"
Cohesion: 0.40
Nodes (5): 2.0.0-beta.5 (2020-10-10), 🐛 Bug Fixes, 🎫 Chores, ♻ Code Refactoring, ⚡ Performance Improvements

### Community 470 - "2.0.0-beta.7 (2020-10-12)"
Cohesion: 0.40
Nodes (5): 2.0.0-beta.7 (2020-10-12), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 471 - "2.0.0-rc.11 (2020-11-18)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.11 (2020-11-18), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 472 - "2.0.0-rc.16 (2021-01-12)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.16 (2021-01-12), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 473 - "2.0.0-rc.1 (2020-10-14)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.1 (2020-10-14), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 474 - "2.0.0-rc.2 (2020-10-17)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.2 (2020-10-17), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 475 - "2.0.0-rc.3 (2020-10-19)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.3 (2020-10-19), 🐛 Bug Fixes, 🎫 Chores, Docs, ✨ Features

### Community 476 - "2.0.0-rc.4 (2020-10-21)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.4 (2020-10-21), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 477 - "2.0.0-rc.5 (2020-10-26)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.5 (2020-10-26), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 478 - "2.0.0-rc.8 (2020-11-2)"
Cohesion: 0.40
Nodes (5): 2.0.0-rc.8 (2020-11-2), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ⚡ Performance Improvements

### Community 479 - "2.0.1 (2021-02-21)"
Cohesion: 0.40
Nodes (5): 2.0.1 (2021-02-21), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 480 - "2.0.2 (2021-03-04)"
Cohesion: 0.40
Nodes (5): 2.0.2 (2021-03-04), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor

### Community 481 - "2.1.1 (2021-03-26)"
Cohesion: 0.40
Nodes (5): 2.1.1 (2021-03-26), 🐛 Bug Fixes, 🎫 Chores, ✨ Features, ✨ Refactor

### Community 482 - "(破坏性更新) Breaking changes"
Cohesion: 0.40
Nodes (5): 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements, ✨ Refactor, (破坏性更新) Breaking changes

### Community 483 - "axios.d.ts"
Cohesion: 0.40
Nodes (4): ErrorMessageMode, RequestOptions, Result, UploadFileParams

### Community 484 - "module.d.ts"
Cohesion: 0.40
Nodes (4): ant-design-vue/es/locale/*, moment/dist/locale/*, virtual:*, *.vue

### Community 485 - "AccountController.cs"
Cohesion: 0.50
Nodes (3): ChallengeAccountController, TuTa.Wms.Web.Controllers, AccountController

### Community 486 - "TestModel.cs"
Cohesion: 0.50
Nodes (3): TuTa.Wms.Models.Test, DateTime, TestModel

### Community 490 - "Default.cshtml"
Cohesion: 0.50
Nodes (3): IHtmlLocalizer<WmsResource>, TuTa.Wms.Localization, WmsResource

### Community 491 - "[2.0.0-rc.15](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.14...v2.0.0-rc.15) (2020-12-31)"
Cohesion: 0.50
Nodes (4): [2.0.0-rc.15](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.14...v2.0.0-rc.15) (2020-12-31), Bug Fixes, Features, Performance Improvements

### Community 492 - "[2.0.0-rc.16](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.15...v2.0.0-rc.16) (2021-01-12)"
Cohesion: 0.50
Nodes (4): [2.0.0-rc.16](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.15...v2.0.0-rc.16) (2021-01-12), Bug Fixes, Features, Performance Improvements

### Community 493 - "[2.0.0-rc.17](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.16...v2.0.0-rc.17) (2021-01-18)"
Cohesion: 0.50
Nodes (4): [2.0.0-rc.17](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.16...v2.0.0-rc.17) (2021-01-18), Bug Fixes, Features, Performance Improvements

### Community 495 - "[2.7.1](https://github.com/anncwb/vue-vben-admin/compare/v2.6.1...v2.7.1) (2021-08-16)"
Cohesion: 0.50
Nodes (4): [2.7.1](https://github.com/anncwb/vue-vben-admin/compare/v2.6.1...v2.7.1) (2021-08-16), Bug Fixes, Features, Performance Improvements

### Community 496 - "2.0.0-beta.6 (2020-10-11)"
Cohesion: 0.50
Nodes (4): 2.0.0-beta.6 (2020-10-11), 🐛 Bug Fixes, ⚡ Performance Improvements, 💄 Styles

### Community 497 - "2.0.0-rc.16 (2020-01-12)"
Cohesion: 0.50
Nodes (4): 2.0.0-rc.16 (2020-01-12), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 498 - "2.0.0-rc.17 (2020-01-18)"
Cohesion: 0.50
Nodes (4): 2.0.0-rc.17 (2020-01-18), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 499 - "2.0.0-rc.18 (2021-02-05)"
Cohesion: 0.50
Nodes (4): 2.0.0-rc.18 (2021-02-05), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 500 - "2.0.3 (2021-03-07)"
Cohesion: 0.50
Nodes (4): 2.0.3 (2021-03-07), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 502 - "2.2.0 (2021-04-06)"
Cohesion: 0.50
Nodes (4): 2.2.0 (2021-04-06), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 503 - "2.4.0 (2021-05-25)"
Cohesion: 0.50
Nodes (4): 2.4.0 (2021-05-25), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 504 - "2.4.1(2021-06-01)"
Cohesion: 0.50
Nodes (4): 2.4.1(2021-06-01), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 505 - "2.4.2(2021-06-10)"
Cohesion: 0.50
Nodes (4): 2.4.2(2021-06-10), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 506 - "2.6.1(2021-07-19)"
Cohesion: 0.50
Nodes (4): 2.6.1(2021-07-19), 🐛 Bug Fixes, 🎫 Chores, ✨ Features

### Community 507 - "2.0.0-beta.56 (2020-10-11)"
Cohesion: 0.50
Nodes (4): 2.0.0-beta.56 (2020-10-11), 🐛 Bug Fixes, ⚡ Performance Improvements, 💄 Styles

### Community 508 - "2.0.0-rc.17 (2021-01-18)"
Cohesion: 0.50
Nodes (4): 2.0.0-rc.17 (2021-01-18), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 509 - "2.0.0-rc.18 (2021-02-05)"
Cohesion: 0.50
Nodes (4): 2.0.0-rc.18 (2021-02-05), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 511 - "2.1.0 (2021-03-15)"
Cohesion: 0.50
Nodes (4): 2.1.0 (2021-03-15), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 512 - "2.2.0 (2021-04-06)"
Cohesion: 0.50
Nodes (4): 2.2.0 (2021-04-06), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 513 - "2.4.0 (2021-05-25)"
Cohesion: 0.50
Nodes (4): 2.4.0 (2021-05-25), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 514 - "2.4.1(2021-06-01)"
Cohesion: 0.50
Nodes (4): 2.4.1(2021-06-01), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 515 - "2.4.2(2021-06-10)"
Cohesion: 0.50
Nodes (4): 2.4.2(2021-06-10), 🐛 Bug Fixes, ✨ Features, ✨ Refactor

### Community 516 - "2.5.2(2021-06-27)"
Cohesion: 0.50
Nodes (4): 2.5.2(2021-06-27), 🐛 Bug Fixes, ✨ Features, ⚡ Performance Improvements

### Community 517 - "2.6.1(2021-07-19)"
Cohesion: 0.50
Nodes (4): 2.6.1(2021-07-19), 🐛 Bug Fixes, 🎫 Chores, ✨ Features

### Community 518 - "(破坏性更新) Breaking changes"
Cohesion: 0.50
Nodes (4): 🐛 Bug Fixes, ✨ Features, ✨ Refactor, (破坏性更新) Breaking changes

### Community 520 - "types.d.ts"
Cohesion: 0.50
Nodes (3): BarMap, BarMapItem, ScrollbarType

### Community 522 - "2.0.0-beta.3 (2020-10-07)"
Cohesion: 0.67
Nodes (3): 2.0.0-beta.3 (2020-10-07), 🎫 Chores, ✨ Features

### Community 523 - "2.0.0-beta.4 (2020-10-08)"
Cohesion: 0.67
Nodes (3): 2.0.0-beta.4 (2020-10-08), 🐛 Bug Fixes, 🎫 Chores

### Community 524 - "2.5.1(2021-06-26)"
Cohesion: 0.67
Nodes (3): 2.5.1(2021-06-26), 🐛 Bug Fixes, ⚡ Performance Improvements

### Community 525 - "2.6.0(2021-07-04)"
Cohesion: 0.67
Nodes (3): 2.6.0(2021-07-04), 🐛 Bug Fixes, ✨ Features

### Community 591 - "package.json"
Cohesion: 0.40
Nodes (4): { name }, license, name, version

### Community 595 - "FeatureProviderDto"
Cohesion: 0.40
Nodes (3): Guid, IEnumerable, CellInfo

### Community 596 - "[2.0.0](https://github.com/anncwb/vue-vben-admin/compare/v2.0.0-rc.18...v2.0.0) (2021-02-17)"
Cohesion: 0.40
Nodes (3): Guid, IEnumerable, WarehouseInfo

### Community 598 - "WmsHttpApiClientModule"
Cohesion: 0.40
Nodes (3): ServiceConfigurationContext, string, WmsHttpApiClientModule

### Community 600 - "SkipStockCtuInDto"
Cohesion: 0.67
Nodes (3): List, SkipStockCtuInDto, StocksCtuInDto

### Community 601 - "tsconfig-paths"
Cohesion: 0.67
Nodes (3): tsconfig-paths, paths, /@/*

### Community 605 - "[2.4.0](https://github.com/anncwb/vue-vben-admin/compare/v2.2.0...v2.4.0) (2021-05-25)"
Cohesion: 0.50
Nodes (4): [2.4.0](https://github.com/anncwb/vue-vben-admin/compare/v2.2.0...v2.4.0) (2021-05-25), Bug Fixes, Features, Performance Improvements

## Knowledge Gaps
- **2163 isolated node(s):** `{ defineConfig }`, `modules`, `mockModules`, `userInfo`, `accountList` (+2158 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **148 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `UpdateRoleInput` connect `ChkResultListEventHandler` to `ILogger`, `Exception.vue`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `IdentityUserUpdateDto` connect `index.vue` to `ValueObject`, `IBarcodeListRepository`, `ErpMaterial`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `UpdateUserInput` connect `IBarcodeListRepository` to `IWarehouseRepository`, `index.vue`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **What connects `{ defineConfig }`, `modules`, `mockModules` to the rest of the system?**
  _2163 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `ErpWorkstationMaterialRequest` be split into smaller, more focused modules?**
  _Cohesion score 0.021021726365237817 - nodes in this community are weakly interconnected._
- **Should `IRepository` be split into smaller, more focused modules?**
  _Cohesion score 0.0519219736087206 - nodes in this community are weakly interconnected._
- **Should `CellDto` be split into smaller, more focused modules?**
  _Cohesion score 0.05558143145474305 - nodes in this community are weakly interconnected._