<template>
    <div class="scan-asn-container">
        ...
        <a-alert
            ...
            show-icon
            message="测试模式"
            :description="`查询使用 Mock 数据（单号 ${TEST_ASN_CODE}），保存调用真实后端接口`"
        />
...
import { ERP_ASNServiceProxy, ErpAsnSaveResponseDto } from '/@/services/ServiceProxies';
import { TEST_ASN_CODE, mockAsn26062444928Data } from './mockAsn26062444928';

interface MockAsnItem {
    ccode?: string;
    darridate?: string;
    cordercode?: string;
    cinvcode?: string;
...
const asnDataList = ref<MockAsnItem[]>([]);
...
function scanAsn() {
    ...
    setTimeout(() => {
        const code = asnCode.value.trim();
        if (code === TEST_ASN_CODE) {
            asnDataList.value = mockAsn26062444928Data;
            message.success(`获取ASN信息成功，共 ${asnDataList.value.length} 条明细`);
        } else {
            message.warning(`测试页仅支持单号 ${TEST_ASN_CODE}`);
        }
        loading.value = false;
    }, 300);
}
...
.modern-input {
    ...
    &:hover {
        ...
        box-shadow: 0 1px 6px rgba(114, 46, 209, 0.2) !important;
    }
