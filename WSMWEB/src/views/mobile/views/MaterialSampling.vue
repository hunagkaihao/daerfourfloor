<template>
  <div class="page-container">
    ...
    <div v-if="stockList.length > 1 && !stockData" class="stock-select-section">
      ...
      />
      <a-row class="button-row">
        <a-button type="primary" @click="finishCheck">抽检完成</a-button>
      </a-row>
    </div>
    ...
    <div v-if="showSuccess" class="success-message">
      <a-alert type="success" message="成功" description="抽检操作已完成" closable @close="showSuccess = false" />
    </div>
  </div>
</template>
...
async function executeOut() {
  ...
  try {
    ...
    const response = await stockInspection(stockData.value.id, actualQty, pagOrBoxCount);
    ...
    if (response && response.success === true) {
      ...
      if (outMode.value === 'box') {
        message.success(`抽检成功 ${outBoxCount.value} 箱(${actualQty} ${stockData.value.unit})`);
      } else {
        message.success(`抽检成功 ${actualQty} ${stockData.value.unit}`);
      }
